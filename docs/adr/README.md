...
| [0027](0027-tiron-joint-speaker-attributed-meeting-transcription.md) | Tiron joint speaker-attributed meeting transcription | Accepted Phase 8 explicit Preview baseline; exact runtime/Preview gate passed on `1c69b61...`, PR #142 merged as `4201c5e7...`, maintainability candidate `fb0985e...` passed but its docs successor exposed a late development-tool advisory, so the patched exact-head replacement, hosted closure including disposable-Windows NSIS, and PR #143 merge remain pending; production promotion remains open |
