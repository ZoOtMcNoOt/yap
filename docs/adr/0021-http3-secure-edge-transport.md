...
**Implementation status:** The gated Phase 5 path implements the transport-neutral durable batch contract over a manually SSH-forwarded loopback HTTP/1.1 development boundary. Phase 7 adds authenticated, revocation-aware, bounded private WebSocket admission on a separate loopback port and a qualified native lower handshake. The desktop can offer a verified fixed-loopback HTTP health origin without saving it or routing audio. It does not implement live ASR, managed LAN/enterprise or live-endpoint discovery, external same-origin WSS/TLS, QUIC/HTTP/3, UDP exposure, or an enterprise edge.
