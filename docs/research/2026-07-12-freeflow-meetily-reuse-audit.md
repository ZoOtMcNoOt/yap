...
|--------|---------------------------|-----------------------------------|
| Island/window/tray | React projects minimized typed overlay state; Tauri owns one initially unfocused, always-on-top, taskbar-free window that accepts explicit keyboard focus, exact native bounds, rounded interaction regions, monitor following, recovery, and close-to-tray | Broader Windows display/elevation evidence and macOS/Linux adapters remain. |
