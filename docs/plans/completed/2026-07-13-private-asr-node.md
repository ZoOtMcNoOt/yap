# Private ASR Node Implementation Record
...
**Current terminology note:** This record used `/triton-cache` for PyTorch's
compiler-generated kernel cache. Current code mounts the same bounded behavior
at `/torch-compile-cache`; only the upstream `TRITON_CACHE_DIR` environment name
remains. This was never a Triton Inference Server dependency.
...
`infra/yap-server-node/gb10-asr-runtime-gate.sh`. Phase 4 ends at one transient,
...
- `server/src/yap_server/pools/batch_asr_worker.py`: bounded mono PCM16/16 kHz
  ...
  attestation.
- `server/src/yap_server/pools/gb10_asr_runtime_gate.py`: immutable artifact checks,
  ARM64/revision image inspection, router-to-pool execution, CUDA assertion,
  WER threshold, and atomic evidence.
- `infra/yap-server-node/gb10-asr-runtime-gate.sh`: clean-head build/run entrypoint
...
4. [x] Run `infra/yap-server-node/gb10-asr-runtime-gate.sh` exactly once on that exact
