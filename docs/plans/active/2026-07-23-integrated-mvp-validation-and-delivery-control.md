...
| Phase 10 — active | PRs #155/#157/#158 merged the Rust provider lifecycle, exact Qwen/Gemma profiles, and bounded already-warm owner-fair admission. Exact Scribe head `e5858424...` passed its private gate and PR #164 merged it as `ec3af506...`. PR #165 merged the no-LLM Archivist core as `2a7ec819...`. Student's exact `428d6e48...` topic-copy prompt repair passed 1,241 portable tests (1,207 passed and 34 declared skips) and private qualification on the unchanged full Qwen rapid profile, with public-safe evidence SHA-256 `f597cca7...`; hosted-green head `b03c6e79...` passed all 12 checks and PR #166 merged the internal core as `2254605e...`. Exact `0970d74c...` remains terminal `deterministic-no-student`. Student product exposure, the unqualified Curator/profile-capacity successors, and the four later workflows (Auditor, Librarian, Analyst, and Coordinator) remain before full-strength two-node residency, mixed-user capacity, observability, release governance, full maintainability/security evidence, deployment, and explicit IT/security/network handoffs. | Each successor uses its own exact-head matrix, hosted checks, reviewed PR, and merge. Qualify the protected profile-capacity and Curator work without reusing historical one-slot/Scribe/Student receipts, deliver the later workflows, and admit a second owned GPU node/private routing because the unchanged Qwen/Gemma profiles do not fit simultaneously on one Spark. External enterprise conformance remains a named handoff when unavailable. |
