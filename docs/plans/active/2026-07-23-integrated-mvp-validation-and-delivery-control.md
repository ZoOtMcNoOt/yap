...
| Phase 10 — active | PRs #155/#157/#158 merged the Rust provider lifecycle, exact Qwen/Gemma profiles, and bounded already-warm owner-fair admission. Exact Scribe head `e5858424...` passed its private gate and PR #164 merged it as `ec3af506...`. PR #165 merged the no-LLM Archivist core as `2a7ec819...`. Student's topic-copy prompt repair is complete-portable-test green on the unchanged full Qwen rapid profile at 1,241 tests (1,207 passed and 34 declared skips). Exact `0970d74c...` returned terminal `deterministic-no-student`; the protected successor needs replacement private qualification. Student product exposure and the five remaining workflows (Curator, Auditor, Librarian, Analyst, and Coordinator) remain before full-strength two-node residency, mixed-user capacity, observability, release governance, full maintainability/security evidence, deployment, and explicit IT/security/network handoffs. | Each role uses its own exact-head matrix, hosted checks, reviewed PR, and merge. Qualify and merge Student, deliver the remaining workflows, and admit a second owned GPU node/private routing because the unchanged Qwen/Gemma profiles do not fit simultaneously on one Spark. External enterprise conformance remains a named handoff when unavailable. |
