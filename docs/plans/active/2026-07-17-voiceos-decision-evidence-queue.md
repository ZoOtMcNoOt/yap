...
| D-03 | Serving split | Cohere batch uses vLLM, Nemotron retains a Transformers reference and evaluates NeMo for server streaming, and ADR 0029 selects vLLM for compatible agent/LLM reasoning and structured tool output. Rust owns orchestration, admission, state, cancellation, and validation. Triton is retired from the current ASR plane. | Exact Phase 6 candidate `a92f338546a2f8bbaded96b04f8987f0ac475c88` passed candidate-safety evidence for vLLM and NeMo without promotion. Phase 8 merged a distinct Tiron meeting Preview without selecting a broad replacement; Phase 9 compares Qwen 3.6 and Nemotron on the checked vLLM runtime, with production integration deferred to Phase 10. | ADR 0025, ADR 0027, ADR 0029, roadmap |
...
- Project approved terminology into the Google OKF glossary/compiler with
  ...
  acceptance.
- Select and benchmark the actual vLLM-compatible reasoning/tool model; prefix
...
| OQ-08 | Which checked vLLM candidate powers governed agents? | Phase 9 task/tool quality, structured-output validity, terminology preservation, prefix-cache isolation, concurrency, latency, cgroup memory, license and governance | No production agent model selected |
