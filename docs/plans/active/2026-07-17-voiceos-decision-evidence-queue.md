...
| D-03 | Serving split | Cohere batch uses vLLM, Nemotron retains a Transformers reference and evaluates NeMo for server streaming, SGLang is reserved for compatible agent/LLM reasoning and structured tool output, and Rust owns orchestration, admission, state, cancellation, and validation. Triton is retired from the current ASR plane. | Exact Phase 6 candidate `c82fe47a683dabd72231ffca377095ff16f2d4f5` passed candidate-safety evidence for vLLM and NeMo without promotion. Provider selection remains Phase 8, SGLang Phase 9, and production integration Phase 10. | ADR 0025, roadmap |
...
| D-16 | NIM/Riva and DGX Spark | A serving product is usable only when its exact model/hardware support matrix and license/deployment contract include DGX Spark. Conflicting marketing/performance data does not override the support matrix. | Deferred challenger; revisit only in its Phase 8 or Phase 10 evidence gate | Phase 5 runtime evaluation, Phase 6 plan |
...
all 14 release-contract checks passed. Historical exact replacement candidate
`c82fe47a683dabd72231ffca377095ff16f2d4f5` subsequently passed the complete
...
independently validated for the exact head and all 30 children. A later
hosted-only teardown-timing contract correction invalidated it as merge
authority. A fresh exact-head gate, hosted closure, PR review, and merge remain
open.
...
| --- | --- | --- | --- |
| OQ-01 | Which exact acoustic-LID model and native format should power required local switching? | Immutable released artifact, compatible commercial use and explicit redistribution boundary, exact locale mapping, representative span accuracy and boundary error, packaging, CPU latency, incremental memory/energy, and ASR interference beside Nemotron | **Selected for the Phase 6 candidate:** NVIDIA AmberNet 1.12.0 static QDQ INT8, exact 29,613,392-byte graph and NeMo-compatible native frontend, three observations, `0.40` margin, full-label abstention, and user-selected Nemotron regional locales. The original natural/noisy gate failed; the product owner deliberately accepted that limitation instead of continuing model research. The later frozen holdout and natural product-route evidence remain recorded without retuning: the natural route detected 0/4 required switches, so automatic switching stays explicit, default-off Preview behavior. The artifact remains verified local-import only while NGC redistribution review is open. Historical exact candidate `c82fe47a683dabd72231ffca377095ff16f2d4f5` passed current-host resource/interference, lifecycle, short-boundary, UI, production-quit, teardown, and the complete Phase 6 matrix, then a hosted-only executable timing correction invalidated it as merge authority. A fresh gate, hosted closure, and merge remain; low-end battery/thermal certification remains default-on or Phase 10 work. |
