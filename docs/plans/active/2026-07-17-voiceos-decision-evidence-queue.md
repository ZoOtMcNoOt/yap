...
| D-03 | Serving split | Cohere batch uses vLLM, Nemotron retains a Transformers reference and evaluates NeMo for server streaming, SGLang is reserved for compatible agent/LLM reasoning and structured tool output, and Rust owns orchestration, admission, state, cancellation, and validation. Triton is retired from the current ASR plane. | Exact Phase 6 candidate `a92f338546a2f8bbaded96b04f8987f0ac475c88` passed candidate-safety evidence for vLLM and NeMo without promotion. Phase 8 merged a distinct Tiron meeting Preview without selecting a broad replacement; SGLang remains Phase 9 and production integration Phase 10. | ADR 0025, ADR 0027, roadmap |
...
| D-23 | Joint speaker-attributed meeting ASR | Use pinned `Trelis/tiron` as the Phase 8 server development baseline. Keep the local anonymous-speaker path, ASR-plus-diarization fallback, Rust result authority, and model-replacement seam. Reproduce its eight window-local/eight released-global limits first; qualify a separate Yap speaker-epoch reconciler before claiming the 32-target/64-ceiling product roster, and expose only observable capacity evidence rather than inventing regions. | Explicit Preview runtime integration executes. Exactly eight meeting-global labels produce a whole-source partial signal; exact window evidence, larger-roster reconciliation, and production promotion remain open. | ADR 0027 and joint speaker-attributed meeting plan |
...
since merged. Its separate maintainability candidate `fb0985e7...` passed, but
the documentation successor exposed a late high-severity development-tool
advisory. The patched lockfile requires a fresh exact-head candidate gate,
hosted closure, and PR #143 merge before Phase 9.
...
- [ ] Promote licensed multi-speaker/overlap evidence, exact window-cap behavior,
  larger-roster speaker epochs, automatic fallback, and purpose-authorized
  reconciliation/naming before claiming production meeting support.
