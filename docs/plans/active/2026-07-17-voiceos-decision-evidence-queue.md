...
| D-03 | Serving split | Cohere batch uses vLLM, Nemotron retains a Transformers reference and evaluates NeMo for server streaming, SGLang is reserved for compatible agent/LLM reasoning and structured tool output, and Rust owns orchestration, admission, state, cancellation, and validation. Triton is retired from the current ASR plane. | Exact Phase 6 candidate `e4a62f4b8914e9233cd5229fa8f134f0c59fdbbb` passed candidate-safety evidence for vLLM and NeMo without promotion. Provider selection remains Phase 8, SGLang Phase 9, and production integration Phase 10. | ADR 0025, roadmap |
...
`e4a62f4b8914e9233cd5229fa8f134f0c59fdbbb` subsequently passed the complete
