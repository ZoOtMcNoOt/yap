...
score.
Phase 7 is active on `feat/phase7-identity-access`. Its server resource boundary
and native client bridge have focused executable evidence. Exactly three
initial adversarial reviews completed; accepted findings are in focused repair
verification and are awaiting bounded re-review by those same reviewers. The frozen exact-candidate matrix,
hosted closure, and merge have not occurred.
...
| [0016](adr/0016-auth-identity-bridge.md) | Canonical Phase 7 decision; biometric and KB persistence sequenced to Phases 8 and 9 | Shared | **150** | The active branch has a bounded official MSAL.NET/WAM sidecar with system-browser fallback and OS-protected cache, tenant-specific account authority and schema-13 ambiguous-binding quarantine, sensitive bearer injection plus protected readiness, strict Entra RS256 issuer/tenant/audience/scope/client/time validation and bounded single-flight JWKS refresh, token-derived `(tid, oid)` request principals, a provider-neutral identity repository with SQLite development storage, read-only steady admission, durable access disable/restore, purpose-control and redacted append-only audit records, owner-scoped job/LID/idempotency/artifact access, non-disclosing 404 behavior, and real asymmetric synthetic two-owner/restart/account-switch evidence | Accepted adversarial repairs are focused-green but bounded re-review remains open; no frozen Phase 7 matrix or reviewed merge; no real tenant registration, enterprise WAM/Conditional Access/MFA/consent conformance, production identity database/audit sink, NativeInterop/Windows SDK distribution approval, authenticated WSS, or enterprise deployment evidence | Complete bounded repair re-review and the exact-candidate Phase 7 gate/hosted closure, then perform real-provider conformance only after IT supplies an approved environment |
...
| [0023](adr/0023-bounded-live-priority.md) | Accepted amendment to ADR 0014's priority rule; historical reference implementation retired | Server | **20** | The bounded live-priority/fairness decision and historical Phase 4 exact-candidate evidence remain documented; current loopback batch commits dispatch directly to the bounded pool | No executing live target, owner-fair router, authenticated owner, durable multi-tenant queue, persistent production service, or measured mixed-load tuning/capacity | Phase 7 supplies the authenticated admission identity; Phase 10 implements and proves the durable live/batch fairness rule against real owners, bounded overload, cancellation isolation, restart recovery, fixed resource ceilings, and sustained mixed-load SLOs |
