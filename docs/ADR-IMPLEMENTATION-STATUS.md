...
**As of:** 2026-08-03; Phases 3–7 and Architecture Checkpoints A/B are merged and
...
The active Phase 8 branch adds an explicitly configured Tiron meeting candidate
through the existing authenticated job and native publication boundaries.
...
| [0020](adr/0020-meeting-capture-diarization-authority.md) | Canonical meeting/capture/identity authority; server baseline selected by 0027 | Shared | **100** | Source-aware sessions/tracks, exact gaps, bounded sinks, crash-safe recording/recovery, one-worker bounded native-drop admission with an exclusive picker lease, gated canonical-WAV upload/reconnect, verified server-authoritative transcript and anonymous-speaker revisions, finite retention, naming restrictions, and one executable Tiron-based server meeting path through native History | No general media conversion, system loopback, promoted local meeting UX/anonymous model, larger-roster speaker reconciliation, contacts, or purpose-authorized identity entry point | Preserve the capture/result authority; complete larger-roster, fallback, identity, and production-promotion evidence under 0020/0027 without raising the score beyond the still-partial ADR scope |
...
| [0027](adr/0027-tiron-joint-speaker-attributed-meeting-transcription.md) | Accepted canonical Phase 8 server Preview baseline; production promotion pending | Server | **150** | Exact current Tiron model/weight/upstream runtime-harness, ECAPA, NVIDIA base-image, Python dependency, source-build, compatibility-patch, license, and public-comparator identities are machine locked. Executable validators bind that lock to the frozen acceptance policy. A digest-pinned ARM64 image installs the upstream whole-meeting runtime; its thin worker verifies local artifacts, runs without networking, and validates source-bounded results. Explicit meeting-only candidate startup verifies both model roots and a receipt-bound checked image, then contract-composes three-hour admission, separate restart-safe and hash-bound transcript/speaker revisions, owner-scoped retrieval, native publication, and History projection. Exact application/runtime head `1c69b61cf2902c9cfda50c6158168890974f969f` passed the supported-launcher client/HTTP/Tiron/native/History roundtrip, rendered four anonymous speaker turns with the separate companion hash bound, and passed the one admitted local/native/server/GB10 Preview matrix against immutable ARM64 image `sha256:19ffb7fbadb95e8332a92ee82ed6a4554e090eeec3d5c680d133c8787dfb4330`. Protected aggregate receipt SHA-256 `9f647b3a968ae31ab4b7f869bda160177b665747a3be5deecdde11399919e154` binds that evidence and reviewed test/gate-only descendant `9ff06d7d...`. The route publishes a source-bound meeting-global `partial` signal when the public aggregate exposes exactly eight labels and records that fallback was not run. | Reviewed exact-head hosted closure, merge, and the post-phase maintainability checkpoint remain open. Private-holdout/scorer reproduction, exact window-cap evidence, speaker epochs, fallback execution, independent quality, full resource/concurrency promotion, and production promotion are explicitly deferred. | Open the focused PR, merge only after the reviewed exact head is green, then perform the separate maintainability checkpoint; keep the score at 150 until the deferred production evidence closes the ADR scope. |
