...
| [0016](adr/0016-auth-identity-bridge.md) | Canonical Phase 7 decision; biometric and KB persistence sequenced to Phases 8 and 9 | Shared | **15** | Evidence/result contracts require provenance for named server assertions; Phase 5 uses an explicit SSH development boundary without treating it as application identity; the active plan freezes the MSAL.NET adapter, server principal, owner-isolation, revocation/purpose-control, and synthetic evidence seams | No MSAL adapter, credential cache, Yap-token validation, identity repository, owner-scoped jobs/LID, grants, revocation, or audit | Execute the active Phase 7 tenant-scoped identity and job authorization plan |
