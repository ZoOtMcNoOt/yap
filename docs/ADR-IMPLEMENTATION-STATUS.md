...
**As of:** 2026-08-02; Phases 3–7 and Architecture Checkpoints A/B are merged and
...
through the existing authenticated job and native publication boundaries. It
passed one exact-head supported-launcher client/HTTP/Tiron/native/History
roundtrip at `6d90a7225a45f181efcb90bb796b1ba97f645eba`, with protected evidence
outside Git. It does not claim model promotion, managed discovery, live ASR,
production SSO, or an enterprise edge.
...
| [0027](adr/0027-tiron-joint-speaker-attributed-meeting-transcription.md) | Accepted canonical Phase 8 server Preview baseline; production promotion pending | Server | **150** | Exact current Tiron model/weight/upstream runtime-harness, ECAPA, NVIDIA base-image, Python dependency, source-build, compatibility-patch, license, and public-comparator identities are machine locked. Executable validators bind that lock to the frozen acceptance policy. A digest-pinned ARM64 image installs the upstream whole-meeting runtime; its thin worker verifies local artifacts, runs without networking, and validates source-bounded results. Focused GB10 short and two-window smokes passed. Explicit meeting-only candidate startup verifies both model roots and a receipt-bound checked image, then contract-composes three-hour admission, separate restart-safe and hash-bound transcript/speaker revisions, owner-scoped retrieval, native publication, and History projection. Exact head `6d90a7225a45f181efcb90bb796b1ba97f645eba` passed a supported-launcher client/HTTP/Tiron/native/History roundtrip that verified the separate speaker companion hash and rendered four anonymous speaker turns; protected evidence remains outside Git. The current branch adds a source-bound meeting-global `partial` signal when the public aggregate exposes exactly eight labels and records that fallback was not run. | Current exact-head roundtrip, one complete Phase 8 Preview gate, reviewed merge, and the post-phase maintainability checkpoint remain open. Private-holdout/scorer reproduction, exact window-cap evidence, speaker epochs, fallback execution, independent quality, full resource/concurrency promotion, and production promotion are explicitly deferred. | Close adversarial findings, rerun the supported roundtrip on the exact candidate, run the one-time Phase 8 Preview gate, merge a green checked head, then perform the separate maintainability checkpoint. |
