...
**As of:** 2026-07-29; Phases 3–6 and Architecture Checkpoints A/B are merged and
...
score.
Phase 7 merged as `66d314d7`; its adversarial checkpoint closed with `ef6d977`.
Executable evidence covers the provider-neutral OIDC verifier with Entra policy,
fail-closed authentication defaults, tenant-scoped ownership, a narrow native
token-provider seam, authenticated bounded private WebSocket admission, and the
qualified native lower handshake. Purpose grants and revocation are implemented
and unit-tested but reachable only from tests: no route, CLI, or operator entry
point calls them, so they are a Phase 8 gate rather than enforced behaviour. No production
...
containment/private-evidence contracts pass. The completed three-lens
working-tree review found Linux lifecycle gaps in ambient Python startup,
control-pipe `SIGPIPE`, pidfd/setup bounds, missing-result recovery, sampler
handle retention, external-container interruption, and stale numeric-PID log
following. Re-review additionally exposed a natural-exit post-reap downgrade,
a zombie-environment read race, unbounded Docker probes, delayed daemon
publication, and created-but-stopped container cleanup. The successor now
latches post-reap proof failures, causally rechecks environment denial, bounds
Docker calls below its supervisor deadline, separates Docker create from start,
and writes an exclusive Docker identity file plus a private recovery record
before creation. It force-removes only a token-verified immutable ID; an
interrupted create with no captured ID remains an explicit cleanup failure and
retains recovery identity instead of treating timed absence as proof. Recovery
artifacts retire only after direct immutable-ID absence; a renamed or relabeled
container is retained as an ownership failure. Retirement succeeds only if all
three private recovery artifacts are actually removed, and the normal lifecycle
gate independently proves their absence before forgetting the proxy path.
Provider containers do not use Docker auto-removal: bounded log capture happens
while the immutable ID remains addressable, followed by explicit exact-ID
removal. Committed/pushed retained-pidfd head
`9defb4a2202b5743f161dafb40f8fb2bc41b8fde` preserved that repair and passed
unaffected focused private prechecks, but connected prequalification rejected
it before admission because it refused GB10's stock
`/usr/bin/socat -> /usr/bin/socat1` package link. The corrected successor
canonicalizes the selected command to its absolute regular executable before
container mutation. Causal and hosted no-skip coverage, all 14 focused proxy
contracts, and the real root-owned GB10 host-path proof execute; the same
architecture, runtime, and assurance reviewers returned GO with no P0–P2
finding. Later exact head `d4adc832da90ef5a65ca8e6a9d702d833e55dbe8`
retained those repairs and copied-package materialization, passed complete
private prequalification, admission, all four private controllers, and
independent 13-child validation. Its sole complete matrix passed every Rust
test, then failed closed because the owned Windows Job retained Visual Studio
Build Tools' `vctip.exe` helper. The head is consumed. The approved
`OptIn=0` change was applied and verified, but a clean default-MSVC link still
launched signed `VCTIP.EXE` from `link.exe`, consistent with Microsoft's
required-diagnostics exception. The successor makes the Build Tools
optional-diagnostics setting a fail-closed 64-bit registry-API prerequisite
rooted in the kernel object-manager SystemRoot and moves native compile/link
evidence outside the product Job to clean exact-head GitHub-hosted Windows VMs.
Actual connector and required WDIO runtime trees remain Job-contained, run
without GitHub credentials, and must prove active-process-zero; cold connector
setup is lock-populated first and the running WDIO binary verifies the reviewed
build SHA. Fresh successor review, prequalification, admission, candidate gate,
hosted native/PR closure, and merge have not occurred. No ADR completion score
changes on focused or failed-gate evidence alone.
...
| [0016](adr/0016-auth-identity-bridge.md) | Canonical Phase 7 decision; biometric and KB persistence sequenced to Phases 8 and 9 | Shared | **150** | The active branch has a provider-neutral OIDC JWT verifier and discovery/JWKS boundary with an Entra-specific tenant/audience/scope/client/role policy; fail-closed default authentication with an explicit development-only loopback mode; token-derived `(tid, oid)` principals; a provider-neutral identity repository with SQLite development storage; durable access disable/restore; role-gated purpose grant/revoke operations and enrollment/matching/adaptation purpose checks that are implemented and unit-tested but called by nothing outside tests; owner-scoped job/LID/idempotency/artifact access; non-disclosing lookup behavior; authenticated bounded private WebSocket admission with expiry/revocation enforcement; and a narrow in-process native token-provider seam that keeps bearer material out of React and ordinary persistence. Exact application/runtime head `dc635916...` passed the complete private 25-cell matrix and independent receipt validation; reviewed descendants through `c1d81fc...` change only hosted/gate tooling, contracts, and documentation. | No approved production native token adapter, real tenant registration, Conditional Access/MFA/consent conformance, production identity database/audit sink, live ASR route, endpoint discovery, external same-origin WSS/TLS/HTTP/3 edge, or IT-owned deployment evidence. Purpose enforcement and access revocation have no HTTP or operator entry point, so `access_disabled` and grants can be changed only by editing `identity.sqlite` directly | Hosted closure and merge are complete at `66d314d7`, with the checkpoint closed at `ef6d977`. Decide in Phase 8 whether to wire the purpose layer to routes or delete it, then perform real-provider conformance only after IT supplies an approved environment and adapter |
