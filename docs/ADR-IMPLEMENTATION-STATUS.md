...
**As of:** 2026-08-11; Phases 3–9 and Architecture Checkpoints A/B are closed and
...
| [0031](adr/0031-eight-agent-voice-os-roster.md) | Accepted complete Phase 10 roster and execution boundary; admission substrate gated and under hosted review | Shared | **50** | The exact eight-role purpose/route/class map is frozen. Protected executable head `7bd93dc6...` implements one Rust-owned bounded multi-user admission state machine, owner-fair scheduling, strict overload/deadline/cancellation/provider-generation behavior, an owner-private Unix broker, and a strict Python adapter. Windows and Linux Rust checks, the Linux socket lifecycle, the exact 169-test portable matrix across 28 modules, and Ruff pass. Replacement qualification admitted both exact routes with public-safe evidence SHA-256 `a75500c3...`; public-lock/aggregate head `135cc2ba...` passed semantic admission, the portable and 17-test Postgres matrices, real restart/retrieval/stale/successor proof, unchanged desktop dependency scope, and exact teardown with public-safe evidence SHA-256 `350c13a5...`. Requests target only already-warm exact routes and never start, swap, or silently substitute a model. | No native/server product endpoint or role workflow consumes the broker. Simultaneous residency, sustained capacity/SLOs, crash-recovery coupling, hosted review, production operations, and IT/security deployment remain open. | Merge the admission slice only after hosted-green exact-head review, then implement and qualify each role as a complete source-bound workflow, starting with Scribe; keep raw/local controls independent. |
