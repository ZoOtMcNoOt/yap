...
| [0024](adr/0024-global-language-routing.md) | Accepted canonical Phase 6 language, local language-diarization, provider, LID, and timing decision; serving amended by 0025 and batch preflight amended by 0026 | Shared | **145** | Bounded catalog/primary/job decisions; durable preprocessing stages; pinned Silero; exact AmberNet 1.12.0 QDQ INT8 local resident detector with one `LiveRuntime` owner, source-time spans, exact-once handoff, visible fallback, private holdout/development-host resource evidence, default-off Preview boundary, and unconsumed fail-closed target-i5 native repeated-session plus release-mode microphone/UI collectors; verify-only server AmberNet batch preflight with an exact frontend/label/artifact lock, five start-to-tail regions, strict all-five agreement, independent client/server decisions, explicit user confirmation, focused real-model/parity/contract tests, and one source-exact ARM64 worker/resource/teardown smoke; source preservation; pinned Nemotron fixed/auto references; fail-closed Cohere timing; immutable label-correction history; an exact-source AMI close/far provider comparison that rejects a universal quality label without promoting a route; and historical rejected candidate evidence. | No second promoted fixed locale/selectable override, consumed target-i5 local interference/sustained-session/energy/thermal evidence, final frozen-head ARM64 batch-preflight repetition, promoted alignment, independently reviewed representative locale/long-meeting gate, complete checked-head accessibility rerun, or complete Phase 6 matrix. The consumed natural-switch target failed and confines local automatic routing to explicit default-off Preview status. | Run and review the accepted local AmberNet target-i5/sustained safety gate, ADR 0026 final frozen-head repetition, ADR 0025 provider gates, timing, accessibility, and the frozen complete matrix on `feat/phase6-preprocessing-pipeline`. Removing Preview requires new independent natural/noisy evidence; authenticated ownership and persistent supervised mixed-load production remain Phases 7 and 10. |
