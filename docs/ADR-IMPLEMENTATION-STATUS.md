...
through PR #157 as `cac8989b...`. Exact local head `9b14beff...` now implements
the bounded multi-user admission substrate; no workflow consumes it yet.
...
| [0031](adr/0031-eight-agent-voice-os-roster.md) | Accepted complete Phase 10 roster and execution boundary; admission substrate implemented locally | Shared | **50** | The exact eight-role purpose/route/class map is frozen. Local executable head `9b14beff...` implements one Rust-owned bounded multi-user admission state machine, owner-fair scheduling, strict overload/deadline/cancellation/provider-generation behavior, an owner-private Unix broker, and a strict Python adapter. Windows and Linux Rust checks, the Linux socket lifecycle, the exact 165-test portable matrix across 28 modules, and Ruff pass. Requests target only already-warm exact routes and never start, swap, or silently substitute a model. | No native/server product endpoint or role workflow consumes the broker. Fresh private route qualification, simultaneous residency, sustained capacity/SLOs, crash-recovery coupling, hosted review, production operations, and IT/security deployment remain open. | Land the admission slice, then implement and qualify each role as a complete source-bound workflow, starting with Scribe; keep raw/local controls independent. |
