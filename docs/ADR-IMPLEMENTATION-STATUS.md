...
**As of:** 2026-08-03; Phases 3–8 and Architecture Checkpoints A/B are merged and
their checked-head evidence remains recorded below. The last post-Phase-8
meeting-transcription maintainability candidate is gated, but its
documentation-only successor failed hosted dependency audit after a new
high-severity development-tool advisory became visible. The patched exact-head
replacement candidate, hosted closure, and PR #143 merge remain. Exact Phase 6
...
`4201c5e7f1674dc0b15e76241bc308c49a5719bb`. The separate ownership and
maintainability candidate `fb0985e7c08cf0a0e69752afbe61e372cbfe76db`
passed its single complete checkpoint matrix, independent receipt validation,
and all required CI and CodeQL jobs. It changes no ADR score or model/runtime
qualification claim. Documentation-only successor `e22368fc...` did not close
the checkpoint: its first hosted frontend audit exposed
`GHSA-mwp4-54f8-5fhr`. The current repair pins patched development-only
`ip-address` 10.3.1; because that changes the lockfile, it requires a fresh
exact-head candidate gate and hosted closure before merge.
...
| [0004](adr/0004-background-diarization-okf-agents.md) | Non-blocking background principles retained; diarization details superseded by 0020 | Shared | **45** | Independent bounded sinks, track/gap/chunk contracts, crash-safe recording, evidence/result types, and the merged ADR 0027 server meeting Preview | No promoted local speaker sink/model, broader diarization/reconciliation, OKF, or agents | Preserve 0020/0027 authority for later speaker promotion; Phase 9 owns OKF/agents |
