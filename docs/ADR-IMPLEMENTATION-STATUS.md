...
| [0024](adr/0024-phase6-global-language-routing.md) | Accepted canonical Phase 6 language, provider, LID, and timing decision | Shared | **50** | Public aggregate candidate benchmarks, a bounded provider/locale/mode/timing schema, artifact-gated Python catalog, separate fail-closed HTTP endpoint, OpenAPI example, native fingerprint/bounds validation, TypeScript projection, and focused cross-language tests exist | No last-known native snapshot, primary-language persistence/UX, VAD stage, LID component, Nemotron server pool/auto mode, aligned-word production, durable stage migration, representative locale gate, or complete Phase 6 matrix | Complete the remaining accepted slices on `feat/phase6-preprocessing-pipeline`, then gate the exact head |
