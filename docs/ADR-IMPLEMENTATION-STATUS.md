...
documentation reconciliation as `fc8a1651...`. Phase 10 Slice 10.1 is an
unmerged locally verified candidate; its hosted Linux proof and merge remain
open.
...
| [0030](adr/0030-rust-supervised-provider-service-lifecycle.md) | Accepted Phase 10 lifecycle target; Slice 10.1 candidate implemented locally | Server | **50** | The unmerged hardware-independent candidate adds one Rust supervisor for one explicit foreground launcher, canonical numeric-loopback and exact single-model readiness, a fixed three-in-60-second child restart policy, bounded stop/reap, owner-private state counters, a hardened systemd outer cgroup, a fail-closed installer, a feature-gated lifecycle fixture, and a dedicated exact-head Linux CI lane. Local format, Clippy, default contracts, five service-lifecycle tests, systemd/installer contracts, Bash syntax, release-cache policy, and production-only binary build are green. | Hosted Linux-only executable-mode and forced-SIGTERM lifecycle proof, exact-head PR/merge, Qwen/Gemma launch profiles, authenticated application admission, simultaneous residency, sustained mixed-owner capacity/p95/p99/fairness/overload/recovery evidence, observability, rollback, SBOM/notices, and IT-owned production deployment remain open. No provider is enabled or promoted. | Keep the score at 50 until the focused hosted-green slice merges. Then bind one exact qualified profile at a time; do not begin capacity or production claims from this hardware-independent lifecycle proof. |
