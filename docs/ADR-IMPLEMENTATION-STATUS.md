...
score.
Phase 7 is active on `feat/phase7-identity-access`. Focused executable evidence
now covers the provider-neutral OIDC verifier with Entra policy, fail-closed
authentication defaults, tenant-scoped ownership, enforced and audited purpose
...
| [0016](adr/0016-auth-identity-bridge.md) | Canonical Phase 7 decision; biometric and KB persistence sequenced to Phases 8 and 9 | Shared | **150** | The active branch has a provider-neutral OIDC JWT verifier and discovery/JWKS boundary with an Entra-specific tenant/audience/scope/client/role policy; fail-closed default authentication with an explicit development-only loopback mode; token-derived `(tid, oid)` principals; a provider-neutral identity repository with SQLite development storage; durable access disable/restore; role-gated purpose grant/revoke operations; enforced and audited enrollment/matching/adaptation purpose checks; owner-scoped job/LID/idempotency/artifact access; non-disclosing lookup behavior; authenticated bounded private WebSocket admission with expiry/revocation enforcement; and a narrow in-process native token-provider seam that keeps bearer material out of React and ordinary persistence. Exact head `d4adc832...` passed all four admitted private controllers and independent 13-child validation before its complete matrix was consumed by a Windows build-tool cleanup failure. | No approved production native token adapter, real tenant registration, Conditional Access/MFA/consent conformance, production identity database/audit sink, live ASR route, endpoint discovery, external same-origin WSS/TLS/HTTP/3 edge, hosted mock-OIDC Docker result, full Phase 7 gate, reviewed PR, or merge | Freeze the disposable-native-build successor and complete the Phase 7/hosted closure, then perform real-provider conformance only after IT supplies an approved environment and adapter |
