...
| [0030](adr/0030-rust-supervised-provider-service-lifecycle.md) | Accepted Phase 10 lifecycle target; Slice 10.1 merged | Server | **100** | The merged hardware-independent baseline adds one Rust supervisor for one explicit foreground launcher, canonical numeric-loopback and exact single-model readiness, a fixed three-in-60-second child restart policy, bounded stop/reap, owner-private state counters, a hardened systemd outer cgroup, a fail-closed installer, a feature-gated lifecycle fixture, and a dedicated exact-head Linux CI lane. Exact hosted-green head `1a487db8...` passed that lane plus every required CI and CodeQL check and merged through PR #155 as `e2d82b89...`. | Qwen/Gemma launch profiles, authenticated application admission, simultaneous residency, sustained mixed-owner capacity/p95/p99/fairness/overload/recovery evidence, observability, rollback, SBOM/notices, and IT-owned production deployment remain open. No provider is enabled or promoted. | Bind one exact qualified profile at a time; do not begin capacity or production claims from this hardware-independent lifecycle proof. |
