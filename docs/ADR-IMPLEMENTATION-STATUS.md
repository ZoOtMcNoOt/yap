...
| [0024](adr/0024-global-language-routing.md) | Accepted canonical Phase 6 language, local language-diarization, provider, LID, and timing decision; serving amended by 0025 and batch preflight amended by 0026 | Shared | **150** | Bounded catalog/primary/job decisions and a visible catalog-derived recording-language selector that currently exposes only gated Cohere `en-US`; durable preprocessing stages; pinned Silero; exact AmberNet 1.12.0 QDQ INT8 local resident detector with one `LiveRuntime` owner, source-time spans, exact-once handoff, visible fallback, private holdout/development-host resource evidence, default-off Preview boundary, and unconsumed fail-closed current-host native repeated-session plus release-mode microphone/UI collectors; verify-only server AmberNet batch preflight with an exact frontend/label/artifact lock, five start-to-tail regions, strict all-five agreement, independent client/server decisions, explicit user confirmation, focused real-model/parity/contract tests, and source-exact ARM64 worker/resource/teardown evidence including the final repetition at `a21964c19e56648e9fddcb5200de419e59a7687c`; source preservation; pinned Nemotron fixed/auto references; fail-closed Cohere timing; immutable label-correction history; the same exact head passing the model-neutral resident-provider candidate-safety lifecycle; an exact-source AMI close/far provider comparison that rejects a universal quality label without promoting a route; historical rejected candidate evidence; and exact head `05d1b82017447df04d46ccc8fa729c5a6a0d0b13` passing the focused integrated Windows desktop/private-server route with durable preprocessing, fixed-language continuity, interruption/recovery, verified result publication, History review, and teardown. | No consumed current-host offline interference/15-minute sustained-session evidence, complete checked-head accessibility rerun, or complete Phase 6 matrix. `wordAlignment` intentionally remains false pending a later selected route's promotion gate. A second Cohere locale is deliberately not manufactured merely to populate the selector. The consumed natural-switch target failed and confines local automatic routing to explicit default-off Preview status. Representative low-end physical battery/thermal certification remains a default-on and Phase 10 release requirement; broad Cohere quality/long-duration work, provider replacement, and independent meeting/overlap promotion remain Phase 8 evidence questions. | Run and review the accepted local AmberNet current-host/15-minute Preview safety gate, accessibility evidence, and the frozen complete matrix on `feat/phase6-preprocessing-pipeline`. Removing Preview requires new independent natural/noisy evidence; authenticated ownership, Cohere-versus-Tiron provider selection, meeting/speaker promotion, persistent supervised mixed-load production, and low-end physical hardware certification remain later gates. |
