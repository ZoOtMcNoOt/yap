...
maintainability checkpoint passed its exact gate, passed hosted CI/CodeQL on
head `84c22ec9...`, and merged through PR #153 as `ca151b1b...`; it does not
inflate the evidence scores below. PR #154 merged the corresponding
documentation reconciliation as `fc8a1651...`. Exact hosted-green head
`1a487db840578d8e415fd2e5a51b1909af4b7041` passed the dedicated Linux
lifecycle lane and every required repository check; PR #155 merged Phase 10
Slice 10.1 as `e2d82b89532addb26fda73f652ae4f68b2127ef7`.
Slice 10.2 passed its exact sequential Qwen/Gemma
lifecycle gate at `4b103c1b...`, fresh route qualification at `4d623212...`, and
...
| [0003](adr/0003-long-term-voice-architecture.md) | Long-term principles retained; runtime, meeting, language, knowledge, and agent details amended by 0013/0014/0017/0019/0020/0022/0024/0028/0029 | Shared | **100** | Capture, local ASR, overlay, history, hotkeys, safe Windows delivery, gated loopback server batch, language evidence, meeting-result authority, deterministic OKF/terminology compilation, permission-safe Postgres/pgvector retrieval, governed tools, and privately qualified vLLM workload routes cover the merged product through Phase 9. The post-Phase-9 checkpoint merged through PR #153. | No promoted multilingual/live server routes, persistent supervised production deployment, simultaneous agent-model residency, sustained mixed-user capacity, external secure edge, enterprise operations, or final release evidence. | Follow the canonical Phase 10 plan rather than the historical phase map. |
...
| [0005](adr/0005-llama-server-agents.md) | Historical solo/local target; team Scribe execution replaced by 0014/0029/0031 | Shared | **20** | The unsafe development renderer-to-Ollama Polish path is removed. The current team Scribe candidate uses the authenticated server rapid route and never exposes provider credentials to the renderer. | No bundled local `llama-server`, local model pin, manager, health gate, packaging, or local correction qualification exists. Raw ASR, not an unqualified local model, is the Scribe fallback. | Keep any future solo/local LLM as a separately qualified product slice; do not restore Ollama or treat this historical design as the executing team route. |
...
| [0029](adr/0029-vllm-agent-reasoning-runtime.md) | Accepted canonical Phase 9 vLLM agent-runtime amendment | Server | **150** | Exact private head `36350d449735a4daea6546e16759f28f6f15631a` returned `required-workload-routes-qualified` with public-safe evidence SHA-256 `ca5a3f712ff737b92cc0d17979e5cd5b00e3034c880729e290a6f6ba255ca951` for locked Qwen 3.6 rapid automation and Gemma 4 complex orchestration. Evidence binds immutable model manifests, parser/template/final-response protocols, semantic tool workloads, citation/terminology/isolation checks, route evidence, cancellation/recovery, cgroup observations, and exact teardown. Exact aggregate candidate `a4f34678...` semantically admitted that hash-locked private tree and passed the complete knowledge gate with evidence SHA-256 `4013903410e22206c5b46f4dfcbf1878badc3dc9bbdfddb0ddad2ba0e2ff3260`; PR #152 merged it. Yap's Python server selector chooses an explicit route and never silently substitutes the other model. Reviewed checkpoint head `a76ed9b0...` retains the same models/routes, freezes two evaluation-only final structural-decoding attempts without tool replay and one complete cited-proposal call, binds Qwen to exact vLLM 26.07 plus XGrammar 0.2.1 while Gemma remains on exact vLLM 26.06, and separates the rapid route's common and proposal qualification bounds while keeping its route maximum 256 and Gemma 512. Its fresh qualification returned `required-workload-routes-qualified` with public-safe evidence SHA-256 `4662a2784510e63da98dcd301ea05ef107196ce46b49d68ad812abdc042d00f0`; schema-3 public lock commit `2cf1e92c...` passed semantic admission; exact aggregate head `22c3f369...` passed the checkpoint gate with evidence SHA-256 `8c2bfdef6b596094fe113a12b1bbfccec94ddeb3944e1b3313f41b61d5df12b0`; hosted head `84c22ec9...` merged through PR #153 as `ca151b1b...`. | Exact `96897d2f...` remains terminal rejected evidence. Rust-owned production orchestration, persistent supervised serving, simultaneous model residency, sustained mixed-route and multi-owner capacity, production p95/p99 SLOs, observability, rollback, external serving, and enterprise deployment remain open. Generic 200+ TPS is not Yap evidence. | Keep the score at 150. Production service and capacity evidence remain Phase 10. |
| [0030](adr/0030-rust-supervised-provider-service-lifecycle.md) | Accepted Phase 10 lifecycle target; Slices 10.1 and 10.2 merged | Server | **100** | The merged hardware-independent baseline adds one Rust supervisor for one explicit foreground launcher, canonical numeric-loopback and exact single-model readiness, a fixed three-in-60-second child restart policy, process-group-complete bounded stop, owner-private state counters, a hardened systemd outer cgroup, a fail-closed installer, a feature-gated lifecycle fixture, and a dedicated exact-head Linux CI lane. Exact hosted-green head `1a487db8...` passed that lane plus every required CI and CodeQL check and merged through PR #155 as `e2d82b89...`. Slice 10.2 binds immutable Qwen/Gemma profiles to separate instances; exact lifecycle head `4b103c1b...` passed both sequential lifecycle/teardown paths with public-safe evidence SHA-256 `9b6a34f6...`, aggregate head `0471b158...` passed after fresh route qualification, and hosted-green head `6d1400cc...` merged through PR #157 as `cac8989b...`. | No provider is enabled or promoted. Simultaneous warm residency, sustained mixed-owner capacity/p95/p99/fairness/overload/recovery evidence, observability, rollback, SBOM/notices, and IT-owned production deployment remain open. | Keep lifecycle authority separate from admission and workflows; do not infer capacity or production readiness from sequential evidence. |
| [0031](adr/0031-eight-agent-voice-os-roster.md) | Accepted complete Phase 10 roster and execution boundary; admission merged, Scribe candidate implemented | Shared | **50** | The exact eight-role purpose/route/class map is frozen. Protected head `7bd93dc6...` implements the Rust-owned owner-fair bounded scheduler, strict overload/deadline/cancellation/provider-generation behavior, owner-private Unix broker, and strict Python adapter; public-lock/gate head `135cc2ba...` qualified both routes and passed semantic admission, Postgres restart/retrieval, and exact teardown. Hosted-green head `cf1e69a4...` passed all 12 required checks and PR #158 merged it as `84d95842...`. The current Scribe candidate is the first authenticated consumer: it accepts finalized source-hashed segments, binds an immutable terminology snapshot, validates structured edits and semantic preservation, keeps the rapid model warm behind the broker, preserves raw ASR, and publishes only a manually accepted immutable correction revision with visible diff. Its public matrix is green. Exact Scribe head `a53333a5...` was correctly rejected because the model response contract omitted one exact trusted request binding. Binding-repair head `b89fd9f1...` proved that binding but was also rejected when edited structured responses exhausted the Scribe-specific 256-token output allowance. Exact 512-token head `21559371...` proved complete responses and retained the multi-owner/latency/preservation/teardown boundaries, but was rejected because model-authored offsets did not consistently bind their source quote. Exact schema-v2 head `cbd7335a...` proved unique server-derived spans and containment; its bounded diagnostic then correctly rejected an unauthorized, reference-unsupported name change and exposed safety-response variability. Exact prompt-grounding head `e62d33e4...` retained safety and teardown but repeated the unsafe name edit. Exact private-use masking head `b80fe0b4...` retained teardown but failed both bounded dispositions; visible-block head `5bc8d10e...` contained exactly but returned malformed JSON and also failed both. The current protected candidate uses an ASCII equal-length block, a 256-character model edit-field cap, shortest-unique-quote instruction, exact block-run restoration, and identical-edit normalization while retaining response schema 2, the model, 512-token allowance, broker, no-retry policy, timeout/deadline, raw-source validators, and thresholds. | Replacement private bilingual/multi-owner Scribe qualification and aggregate admission are pending. Archivist, Student, Curator, Auditor, Librarian, Analyst, and Coordinator are not delivered. Simultaneous residency, sustained capacity/SLOs, production operations, and IT/security deployment remain open. | Qualify and merge Scribe on one exact head, then implement each remaining role as its own source/permission/persistence-bound workflow before mixed-route capacity and enterprise gates. |
