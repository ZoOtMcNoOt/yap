...
| [0031](adr/0031-eight-agent-voice-os-roster.md) | Accepted complete Phase 10 roster and execution boundary; admission merged, Scribe qualification passed | Shared | **50** | The exact eight-role purpose/route/class map is frozen. Protected head `7bd93dc6...` implements the Rust-owned owner-fair bounded scheduler, strict overload/deadline/cancellation/provider-generation behavior, owner-private Unix broker, and strict Python adapter; public-lock/gate head `135cc2ba...` qualified both routes and passed semantic admission, Postgres restart/retrieval, and exact teardown. Hosted-green head `cf1e69a4...` passed all 12 required checks and PR #158 merged it as `84d95842...`. Scribe is the first authenticated consumer: it accepts finalized source-hashed segments, binds an immutable terminology snapshot, validates structured edits and semantic preservation, keeps the rapid model warm behind the broker, preserves raw ASR, and publishes only a manually accepted immutable correction revision with visible diff. Exact source-lock head `e5858424...` passed its 24-case bilingual/eight-owner private qualification gate with public-safe semantic evidence SHA-256 `5e187ed4...`, correction benefit, exact uncertainty/raw fallback, one stable warm rapid generation, and zero residue. Historical rejected attempts remain terminal and were not reused. | Hosted Scribe review/merge, Archivist, Student, Curator, Auditor, Librarian, Analyst, and Coordinator are not delivered. Simultaneous residency, sustained capacity/SLOs, production operations, and IT/security deployment remain open. | Merge the exact-qualified Scribe slice, prove the two-route warm multi-user capacity boundary, then implement each remaining role as its own source/permission/persistence-bound workflow before enterprise gates. |
