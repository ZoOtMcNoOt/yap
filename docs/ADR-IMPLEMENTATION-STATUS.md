...
| [0027](adr/0027-tiron-joint-speaker-attributed-meeting-transcription.md) | Accepted canonical Phase 8 server development baseline; production promotion pending | Server | **0** | Immutable Tiron model/weight/harness identities, server/local/fallback boundaries, eight-slot window versus dynamic session-roster semantics, messy-meeting evidence contract, and ordered Phase 8 plan are documented | No complete dependency/license lock, Tiron/ECAPA worker, Rust meeting adapter, speaker-attributed scorer, public reproduction, independent holdout result, GB10 resource/concurrency/lifecycle evidence, or production result publication | After Phase 7 and its checkpoint merge, freeze the corpus/thresholds first, implement the bounded worker and validated result path, then run the one-time Phase 8 gate |
