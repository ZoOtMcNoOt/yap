...
| [0031](adr/0031-eight-agent-voice-os-roster.md) | Accepted complete Phase 10 roster and execution boundary; admission, Scribe, and Archivist merged; Student topic-copy prompt repair awaits private qualification | Shared | **50** | The exact eight-role purpose/route/class map is frozen. Protected head `7bd93dc6...` implements the Rust-owned owner-fair bounded scheduler, strict overload/deadline/cancellation/provider-generation behavior, owner-private Unix broker, and strict Python adapter; public-lock/gate head `135cc2ba...` qualified both routes and passed semantic admission, Postgres restart/retrieval, and exact teardown. PR #158 merged that substrate as `84d95842...`. Exact Scribe source-lock head `e5858424...` passed its private gate with public-safe semantic evidence SHA-256 `5e187ed4...`; hosted-green head `bc9a88bc...` passed all 12 checks and PR #164 merged Scribe as `ec3af506...`. PR #165 merged the no-LLM Archivist core as `2a7ec819...` after exact hosted head `e1899db7...` passed all 12 checks. Student's exact-one server-owned evidence contract remains intact; its prompt now forbids topic-derived subjects absent from the selected quote/evidence. Exact `0970d74c...` ran 1,241 portable tests (1,207 passed and 34 declared skips) but returned terminal `deterministic-no-student` with public-safe evidence SHA-256 `316631d5...`; the protected successor is complete-portable-test green at the same 1,241-test total and needs fresh private qualification. | Student qualification/product exposure/hosted merge, Curator, Auditor, Librarian, Analyst, and Coordinator are not delivered. One Spark cannot keep both unchanged full profiles resident simultaneously; a second owned GPU node/private routing is required before warm two-route promotion. Sustained capacity/SLOs, production operations, and IT/security deployment remain open. | Qualify and merge the repaired Student core, implement each remaining role as its own source/permission/persistence-bound workflow, and admit a full-strength two-node topology before enterprise gates. |
