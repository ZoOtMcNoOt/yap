...
| [0008](adr/0008-speechbrain-lid-gate.md) | User-gated LID principle retained; runtime, probes, and threshold amended by 0024 | Shared | **0** | Aggregate candidate/probe research exists; only generic validated language hints execute | No isolated LID component, artifact lock, durable evidence, picker, user confirmation, or runtime tests | Implement ADR 0024's bounded server-side batch preflight |
...
| [0014](adr/0014-server-tier-compute-topology.md) | Canonical server topology, amended/constrained by 0016/0019/0020/0021/0023/0024 | Shared | **150** | Versioned contracts, bounded loopback capability API, validated connector with serialized settings publication, durable desktop/server job state, canonical-WAV preparation, descriptor-bound exact artifact reads, reconnect drain, verified result publication, hardened bootstrap, owner-fair router, immutable Cohere/NGC/Python 3.12 lock, verified ASR catalog projection, and the gated GB10 Phase 5 vertical slice | No additional Phase 6 model pools, general media conversion, persistent production service, authenticated owner/router, WSS/live pool, TLS/QUIC edge, long-recording benchmark, or measured multi-worker capacity | Preserve the gated batch baseline; complete preprocessing under ADR 0024, then later identity/live/capacity/enterprise gates |
...
| [0024](adr/0024-phase6-global-language-routing.md) | Accepted canonical Phase 6 language, provider, LID, and timing decision | Shared | **100** | Public aggregate candidate benchmarks; a bounded artifact-gated provider catalog; separate fail-closed endpoint; native fingerprint/bounds validation; an origin-bound atomic/no-follow last-known snapshot that cannot authorize offline choices; versioned primary-language persistence; explicit OS-suggestion confirmation; catalog-derived Settings/import UI; immutable per-job language disposition in SQLite, capture manifest, and server create contract; legacy request-idempotency compatibility; and focused Rust/TypeScript/Python contract tests exist | No second promoted fixed locale, VAD stage, LID component, Nemotron server pool/auto mode, aligned-word production, complete durable stage model, representative locale gate, or complete Phase 6 matrix | Complete the remaining accepted slices on `feat/phase6-preprocessing-pipeline`, then gate the exact head |
