...
| [0024](adr/0024-global-language-routing.md) | Accepted canonical Phase 6 language, local language-diarization, provider, LID, and timing decision; serving amended by 0025 and batch preflight amended by 0026 | Shared | **145** | Bounded catalog/primary/job decisions; durable preprocessing stages; pinned Silero; exact AmberNet 1.12.0 QDQ INT8 local resident detector with one `LiveRuntime` owner, source-time spans, exact-once handoff, visible fallback, private holdout/development-host resource evidence, and default-off Preview boundary; verify-only server AmberNet batch preflight with an exact frontend/label/artifact lock, five start-to-tail regions, strict all-five agreement, independent client/server decisions, explicit user confirmation, and focused real-model/parity/contract tests; source preservation; pinned Nemotron fixed/auto references; fail-closed Cohere timing; immutable label-correction history; and historical rejected candidate evidence. | No second promoted fixed locale/selectable override, target-i5 local interference/sustained-session evidence, exact checked-head ARM64 batch-preflight resource result, promoted alignment, representative locale gate, complete checked-head accessibility rerun, or complete Phase 6 matrix. The consumed natural-switch target failed and confines local automatic routing to explicit default-off Preview status. | Finish the accepted local AmberNet target-i5/sustained safety gate, ADR 0026 ARM64/resource gate, ADR 0025 provider gates, timing, accessibility, and the frozen complete matrix on `feat/phase6-preprocessing-pipeline`. Removing Preview requires new independent natural/noisy evidence; authenticated ownership and persistent supervised mixed-load production remain Phases 7 and 10. |
