...
does not claim model promotion, managed discovery, live ASR, production SSO, or
an enterprise edge.
...
| [0027](adr/0027-tiron-joint-speaker-attributed-meeting-transcription.md) | Accepted canonical Phase 8 server development baseline; production promotion pending | Server | **100** | Exact current Tiron model/weight/upstream runtime-harness, ECAPA, NVIDIA base-image, Python dependency, source-build, compatibility-patch, license, and public-comparator identities are machine locked. Executable validators bind that lock to the frozen acceptance contract. A digest-pinned ARM64 image installs the upstream whole-meeting runtime; its thin worker verifies local artifacts, runs without networking, and validates source-bounded results. Focused GB10 short and two-window smokes passed. Explicit meeting-only candidate startup verifies both model roots and a receipt-bound checked image, then contract-composes three-hour admission, separate restart-safe and hash-bound transcript/speaker revisions, owner-scoped retrieval, native publication, and History projection. | No real supported-launcher HTTP/native roundtrip, private-holdout seal, executed scorer/public reproduction, speaker-epoch reconciler, independent quality result, full resource/concurrency/lifecycle evidence, complete Phase 8 gate, or production promotion | Seal the independent holdout, reproduce and score the frozen evidence, close larger-roster behavior, review the candidate path, and run the one-time Phase 8 gate |
