...
| [0024](adr/0024-phase6-global-language-routing.md) | Accepted canonical Phase 6 language, provider, LID, and timing decision | Shared | **100** | Public aggregate candidate benchmarks; a bounded artifact-gated provider catalog; separate fail-closed endpoint; native fingerprint/bounds validation; versioned atomic/no-follow primary-language persistence; explicit OS-suggestion confirmation; catalog-derived Settings/import UI; immutable per-job language disposition in SQLite, capture manifest, and server create contract; legacy request-idempotency compatibility; and focused Rust/TypeScript/Python contract tests exist | No last-known native catalog snapshot, second promoted fixed locale, VAD stage, LID component, Nemotron server pool/auto mode, aligned-word production, complete durable stage model, representative locale gate, or complete Phase 6 matrix | Complete the remaining accepted slices on `feat/phase6-preprocessing-pipeline`, then gate the exact head |
