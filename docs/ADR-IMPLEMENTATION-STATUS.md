...
| [0031](adr/0031-eight-agent-voice-os-roster.md) | Accepted complete Phase 10 roster and execution boundary; four role cores merged through PR #168; Librarian qualified candidate; three later workflows | Shared | **50** | The exact eight-role map is frozen. PR #168 merged Curator/profile-capacity. Exact `56b7f5d0...` privately qualified the no-LLM Librarian candidate with public-safe evidence SHA-256 `def8e648...`: eight owners entered the actual synchronized broker wave; ten exact invocations, permission suppression/revocation/stale/cancel/deadline checks, Server-IO one-active/second-queued containment, PostgreSQL restart/read-back, exact audits, zero proposals, and teardown passed. Exact predecessor `ecdcb8ee...` is terminal/inadmissible because only seven broker submissions occurred. | Four role cores remain merged. Librarian is unmerged and has no HTTP/native/renderer/UI product surface; hosted review/merge remains pending. Analyst, Coordinator, and Auditor remain unimplemented. This is not simultaneous Qwen/Gemma residency, sustained capacity/fairness, production SLO, or deployment evidence. | Merge only after hosted-green exact-head review; then implement Analyst, Coordinator, and Auditor and admit the full-strength two-node topology before enterprise gates. |
