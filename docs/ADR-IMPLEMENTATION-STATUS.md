...
and native client bridge have focused executable evidence, but the exact
candidate review, complete matrix, hosted closure, and merge have not occurred.
...
| [0016](adr/0016-auth-identity-bridge.md) | Canonical Phase 7 decision; biometric and KB persistence sequenced to Phases 8 and 9 | Shared | **150** | The active branch has a bounded official MSAL.NET/WAM sidecar with system-browser fallback and OS-protected cache, a Rust token/account-binding owner with sensitive bearer injection, strict Entra RS256 issuer/tenant/audience/scope/client/time validation and bounded single-flight JWKS refresh, token-derived `(tid, oid)` request principals, a provider-neutral identity repository with SQLite development storage, principal upsert, access revocation, purpose-control and redacted append-only audit records, owner-scoped job/LID/idempotency/artifact access, non-disclosing 404 behavior, durable desktop account binding, and real asymmetric synthetic two-owner/restart/account-switch evidence | No frozen Phase 7 matrix or reviewed merge; no real tenant registration, enterprise WAM/Conditional Access/MFA/consent conformance, production identity database/audit sink, NativeInterop distribution approval, authenticated WSS, or enterprise deployment evidence | Complete the exact-candidate Phase 7 review/gate/hosted closure, then perform real-provider conformance only after IT supplies an approved environment |
