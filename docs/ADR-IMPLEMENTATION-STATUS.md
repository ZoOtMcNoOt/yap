...
**As of:** 2026-07-23; Phases 3–5 and Architecture Checkpoint A are merged and their checked-head evidence remains recorded below. Phase 5 PR head `4771d9be60562fa009ccecbcd3c7111b699883a5` passed the one-time complete local/native/server/GB10 gate and hosted checks, then merged as `b6677631b2cc8283f0f6466622f2dfa7cfdb38f6`. Checkpoint implementation candidate `6d55816b0406a2365376d7b2d9a7da2afecf9118` passed its one-time complete matrix; final PR head `2dc1c48c31928106d07cc638828f055929c33e0c` passed hosted CI, CodeQL, and disposable-Windows NSIS, then merged as `a80934d844a068110e7f86b30b6e29d35146db57`. Phase 6 is active; ADR 0024 has focused executable catalog, fixed-language decision, local primary-language conditioning, durable-stage, normalization, advisory-VAD, local language-span, and released-candidate slices. ADR 0025 replaces the retired Triton experiment with focused Cohere vLLM and resident Nemotron NeMo contracts. Exact executable commit `2caf1969000154ffba24511a5c35b57f7f975036` also completed a production-preprocessed 17.49-minute AMI close/far comparison that favored Nemotron lexical accuracy and Cohere throughput/punctuation without promoting either route. ADR 0026 replaces the executing SpeechBrain preflight with a verify-only AmberNet five-region client/server path under focused tests; exact executable head `a21964c19e56648e9fddcb5200de419e59a7687c` passed its final source-exact ARM64 image/resource/lifecycle repetition. Exact executable head `05d1b82017447df04d46ccc8fa729c5a6a0d0b13` then passed the focused connected Windows desktop/private-server slice through durable preprocessing, a tunnel interruption, the advertised Cohere fixed route, server-authoritative result publication, verified History review, and complete remote teardown. The automatic local route is implemented only as an explicit, default-off Preview after its representative natural-switch quality target failed. The per-job selector intentionally exposes only gated Cohere `en-US`; a second Cohere locale is not a Phase 6 closure requirement while replacement remains an evidence-gated Phase 8 question. Phase 6 has not passed its complete gate or promoted the Cohere vLLM or Nemotron NeMo services.

Exact executable head `74322bf42c22058ffa88620f28cd4bf118ad8c01`
also passed the focused current-host no-server target-client qualification with
12/12 paced cycles, all nine short-boundary cases, the unattended release-mode
UI lifecycle, and complete recording/model/process/listener teardown.

Exact GB10 head `a21964c19e56648e9fddcb5200de419e59a7687c` passed ADR 0025's
composed 18-child resident-provider candidate-safety lifecycle with exact
teardown. This closes that Phase 6 component only; representative provider
quality, promotion, and the Tiron replacement decision remain Phase 8 work.
...
| [0014](adr/0014-server-tier-compute-topology.md) | Canonical server topology, amended/constrained by 0016/0019/0020/0021/0023/0024/0025/0027 | Shared | **150** | Versioned contracts, bounded loopback capability API, validated connector with serialized settings publication, durable desktop/server job state, canonical-WAV preparation, descriptor-bound exact artifact reads, reconnect drain, verified result publication, hardened bootstrap, fixed-development-owner router seam, immutable Cohere/NGC/Python 3.12 locks, verified ASR catalog projection, the gated one-running/two-queued raw Transformers GB10 slice, pinned Nemotron reference adapters, resident Cohere vLLM and NeMo workers/services/images/launchers, privacy-safe repeated-fixture duration controls through the exact four-hour boundary, exact head `a21964c19e56648e9fddcb5200de419e59a7687c` passing their composed 18-child candidate-safety lifecycle and teardown, and an exact-source production-preprocessed AMI close/far long-meeting comparison; retired Triton results remain negative evidence | No provider has been promoted; no selected persistent warm production pool, general media conversion, authenticated owner/router, WSS/live pool, TLS/QUIC edge, independently reviewed representative long-recording promotion benchmark, sentinel-rich long-form quality result, rollback result, or measured multi-worker/mixed-load capacity exists | Preserve the gated provider-neutral boundary; derive the authenticated owner/admission seam in Phase 7; implement and compare the ADR 0027 Tiron meeting worker in Phase 8; implement external batch/WSS transport, persistent warm pools, observability, and measured multi-owner mixed-load capacity in Phase 10 |
...
| [0024](adr/0024-global-language-routing.md) | Accepted canonical Phase 6 language, local language-diarization, provider, LID, and timing decision; serving amended by 0025 and batch preflight amended by 0026 | Shared | **180** | Bounded catalog/primary/job decisions and a visible catalog-derived recording-language selector that currently exposes only gated Cohere `en-US`; durable preprocessing stages; pinned Silero; exact AmberNet 1.12.0 QDQ INT8 local resident detector with one `LiveRuntime` owner, source-time spans, exact-once handoff, visible fallback, private holdout/development-host resource evidence, and default-off Preview boundary; exact executable head `74322bf42c22058ffa88620f28cd4bf118ad8c01` passing the focused no-server target-client qualification with 12/12 paced native cycles, all nine short-boundary cases, unattended release-mode capture/UI, cancellation recovery, save/delete, production quit, and complete recording/model/process/listener teardown; verify-only server AmberNet batch preflight with an exact frontend/label/artifact lock, five start-to-tail regions, strict all-five agreement, independent client/server decisions, explicit user confirmation, focused real-model/parity/contract tests, and source-exact ARM64 worker/resource/teardown evidence including the final repetition at `a21964c19e56648e9fddcb5200de419e59a7687c`; source preservation; pinned Nemotron fixed/auto references; fail-closed Cohere timing; immutable label-correction history; the same exact head passing the model-neutral resident-provider candidate-safety lifecycle; an exact-source AMI close/far provider comparison that rejects a universal quality label without promoting a route; historical rejected candidate evidence; and exact head `05d1b82017447df04d46ccc8fa729c5a6a0d0b13` passing the focused integrated Windows desktop/private-server route with durable preprocessing, fixed-language continuity, interruption/recovery, verified result publication, History review, and teardown. | No complete checked-head accessibility rerun or complete Phase 6 matrix. `wordAlignment` intentionally remains false pending a later selected route's promotion gate. A second Cohere locale is deliberately not manufactured merely to populate the selector. The consumed natural-switch target failed and confines local automatic routing to explicit default-off Preview status. A longer manual physical-device soak and representative low-end battery/thermal certification remain default-on and Phase 10 release requirements; broad Cohere quality/long-duration work, provider replacement, and independent meeting/overlap promotion remain Phase 8 evidence questions. | Freeze and adversarially review the Phase 6 candidate, then run the accessibility-inclusive complete matrix exactly once. Removing Preview requires new independent natural/noisy evidence; authenticated ownership, Cohere-versus-Tiron provider selection, meeting/speaker promotion, persistent supervised mixed-load production, and low-end physical hardware certification remain later gates. |
| [0025](adr/0025-provider-specific-asr-serving.md) | Accepted amendment replacing the universal Triton ASR plane with provider-specific runtimes | Server | **150** | Digest-pinned NVIDIA vLLM 26.06/Python 3.12 Cohere image contract; attributed TorchAudio Mel-filter shim; exact upstream PyTorch finalizer backport with BSD-3-Clause provenance and pinned-source assertions; strict loopback/API-key/version/model readiness boundaries; bounded request/response validation; explicit socket interruption with observed vLLM engine abort; provider-neutral Cohere and Nemotron workers; pinned Cohere vLLM and native NeMo cache-aware schedulers with checked prompt projection, cancellation, admission, and resource controls; source-exact full-model image/adapter/teardown smokes; checked launchers with bounded host-loopback proxy ownership and no Docker-published provider port; exact head `a21964c19e56648e9fddcb5200de419e59a7687c` passing the composed 18-child duration/request/language/resource/lifecycle gate with exact teardown and bounded aggregate SHA `a6931acc127f2ca74e6d3a4c8c9aa6c93e33289f1d1312a4626d659dfcbeb9cb`; and exact head `05d1b82017447df04d46ccc8fa729c5a6a0d0b13` completing the checked Cohere route from real desktop import through server-authoritative result and cleanup; retired Triton implementation remains removed | Host-proxy CPU/RSS is outside the provider cgroup profile; neither service is promoted; independently reviewed representative quality, frozen percentile, output-behavior, rollback, whole-host capacity, and persistent supervised mixed-user production evidence do not exist | Preserve provider-neutral job/result authority and the passed candidate-safety boundary; leave broad provider promotion and Cohere-versus-Tiron replacement to Phase 8, authenticated ownership to Phase 7, and persistent production supervision/capacity to Phase 10 |
| [0026](adr/0026-ambernet-batch-language-preflight.md) | Accepted replacement for the superseded SpeechBrain batch preflight | Shared | **100** | Verify-only exact AmberNet 1.12.0 INT8 QDQ artifact contract; pinned Python 3.12/NumPy/CPU ONNX Runtime image; independent NeMo-compatible frontend golden and AMD64/ARM64 logit parity; one-thread graph contract; deterministic five-region start-to-tail selection through the four-hour selector bound; strict all-five agreement/manual fallback; bounded private materialization/transport/admission/cancellation/cleanup; independent Rust response recomputation; focused real-model, Python, Rust, and infra tests; focused ARM64 evidence at `c6862262fa36a83bcd40a7bffa65ec6429ec097e`; final source-exact production-worker repetition at `a21964c19e56648e9fddcb5200de419e59a7687c` with 111,902,720 peak cgroup bytes, six peak PIDs, 670,672 CPU microseconds, 788 ms wall time, zero memory events, and clean teardown; and exact head `05d1b82017447df04d46ccc8fa729c5a6a0d0b13` advertising the component through the canonical batch launcher while the short-input policy preserved and confirmed the fixed job decision without unnecessary inference dispatch. | Representative advertised-locale/long-tail suggestion evidence and the complete Phase 6 gate remain open. Redistribution is not approved, so model delivery remains explicit verify-only import. | Close the remaining bounded Phase 6 evidence without adding download/bundle behavior or claiming four-hour end-to-end ASR. |
