...
| [0020](adr/0020-meeting-capture-diarization-authority.md) | Canonical meeting/capture/identity authority; server baseline selected by 0027 | Shared | **100** | Source-aware sessions/tracks, exact gaps, bounded sinks, crash-safe recording/recovery, one-worker bounded native-drop admission with an exclusive picker lease, gated canonical-WAV upload/reconnect, verified server-authoritative transcript and anonymous-speaker revisions, finite retention, naming restrictions, one executable Tiron-based server meeting path through native History, and an active source-time 32-target/64-ceiling reconciliation candidate | No general media conversion, system loopback, promoted local meeting UX/anonymous model, contacts, purpose-authorized identity entry point, or passed production-promotion evidence | Preserve the capture/result authority; finish meeting quality/runtime promotion and later purpose-authorized identity under 0020/0027 without raising the score before evidence exists |
...
| [0027](adr/0027-tiron-joint-speaker-attributed-meeting-transcription.md) | Accepted canonical Phase 8 server baseline; production promotion pending | Server | **150** | Exact Tiron model/runtime/ECAPA/container/dependency and public-comparator identities are machine locked. The merged Preview route, immutable revisions, owner-scoped publication, History projection, and historical gates remain proven. The active promotion candidate replaces the capped aggregate product contract with exact 30-second public-API epochs, request-scoped ECAPA reconciliation, canonical 32-target/64-ceiling session IDs, typed decode-window/session capacity records, strict `Unknown`, and a fully parsed frozen promotion policy. It removes the obsolete caller-selected speaker limit and duplicate fallback requirement. | The independently reviewed private holdout is not configured; the evaluator must still consume hash-bound candidate/image/review/scorer/slice/runtime children rather than caller-authored aggregates; public/scorer reproduction, production GB10 evidence, exact-head outcome, hosted closure, and promotion remain open. | Keep the score at 150. Finish one reviewed production-promotion gate on the frozen exact head; promote only a passing meeting-only outcome, otherwise retain `unadvertised-baseline`, then begin Phase 9 separately. |
