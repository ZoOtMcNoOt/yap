...
their checked-head evidence remains recorded below. Historical Phase 6
executable candidate `c82fe47a683dabd72231ffca377095ff16f2d4f5` passed its frozen
...
provider is promoted. Hosted Windows then exposed a test-only teardown-timing
portability defect. Its executable correction invalidated `c82fe47...` as
merge authority while preserving that receipt as historical evidence. The
repaired implementation passed the required bounded three-agent adversarial
re-review; a fresh exact-head 30-child gate, hosted closure, PR review, and
merge remain.
