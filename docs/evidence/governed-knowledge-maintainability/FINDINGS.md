...
| KAP-07 | P1 | Knowledge authority/persistence | Compiled generation identity -> staging -> activation | A caller could mutate a compiled permission, concept, chunk, or relationship while retaining the old generation SHA; SQL rows could also be changed after staging because activation checked only counts/embeddings. | Implemented; focused portable and real-Postgres proofs green; final gate and independent re-review pending | The compiler contract owns one canonical serializer and validator, including per-concept profile/resource/content-digest checks. Compilation, source admission, staging, and activation recompute permission/projection/aggregate identities. Activation reconstructs the persisted non-embedding generation and revalidates its durable source admission under the exclusive tenant lock, rejecting every tested admission/policy/audience/concept/chunk/relationship mutation while preserving the prior active generation. |
