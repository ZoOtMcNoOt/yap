...
| KAP-03 | P1 | Knowledge authority/persistence | Reviewed/curated source admission -> compilation -> staging | Reviewed captures and compiled builds lacked an executable admission link. The first admission repair still allowed a caller-authored compiled path/body/frontmatter/permission bundle and a raw curated reviewer/manifest label. | Implemented; focused real-Postgres proof green; final gate and independent re-review pending | Reviewed admission accepts authenticated owner authority, parses the stored immutable source, exact-compares path/concept/resource/frontmatter/body/provenance, and enforces the current owner-only permission. Curated review requires the fixed authenticated curator role and derives its manifest from the canonical generation. Durable admission rows rehash on read; unreviewed, forged, internally rehashed, path/resource, cross-owner, and stored-row tamper cases fail. |
| KAP-07 | P1 | Knowledge authority/persistence | Compiled generation identity -> staging -> activation | A caller could mutate a compiled permission, concept, chunk, or relationship while retaining the old generation SHA; SQL rows could also be changed after staging because activation checked only counts/embeddings. | Implemented; focused portable and real-Postgres proofs green; final gate and independent re-review pending | The compiler contract owns one canonical serializer and validator. Compilation, source admission, staging, and activation recompute permission/projection/aggregate identities. Activation reconstructs the persisted non-embedding generation under the exclusive tenant lock and rejects every tested policy/audience/concept/chunk/relationship mutation while preserving the prior active generation. |
...
| ARCH-04 | P2 | Architecture/human maintainability | Test ownership/cohesion | One 700+ line module mixed aggregate-gate contracts with owned Postgres/Docker lifecycle tests and fakes. | Implemented; exact current membership frozen | Gate contracts and owned Postgres runtime tests now live in separate functional modules. The checkpoint portable contract lists 24 modules/129 tests; the database contract lists 4 modules/17 tests. |
