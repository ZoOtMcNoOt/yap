...
| AR-02 | P1 | Agent runtime/evidence | Governed MCP database cancellation | MCP abandoned its thread on cancellation, while a daemon watcher lost `cancel_safe` failure and did not require completion. The first bounded-wait repair could still return while a non-acknowledging database worker survived. | Implemented; focused verification green; final re-review pending | A non-daemon watcher reports cancel/close failure and is joined exactly. MCP signals cancellation and shield-waits until the database worker, watcher, and connection context have actually exited; it never reports cancellation while work survives. Non-acknowledging and blocked-close regressions prove the caller remains pending until containment completes. |
| KAP-03 | P1 | Knowledge authority/persistence | Reviewed/curated source admission -> compilation -> staging | Reviewed captures and compiled builds lacked an executable admission link. The first admission repair still allowed a caller-authored compiled path/body/frontmatter/permission bundle and a raw curated reviewer/manifest label. | Implemented; focused real-Postgres proof green; final gate and independent re-review pending | Reviewed admission accepts authenticated owner authority, parses the stored immutable source, exact-compares path/concept/resource/frontmatter/body/provenance, and enforces the current owner-only permission. Curated admission is one durable operation that accepts only a server-authenticated principal with the fixed curator role and derives its review/manifest identities internally. Durable admission rows rehash on read; unreviewed, forged, internally rehashed, path/resource, cross-owner, and stored-row tamper cases fail. |
| KAP-07 | P1 | Knowledge authority/persistence | Compiled generation identity -> staging -> activation | A caller could mutate a compiled permission, concept, chunk, or relationship while retaining the old generation SHA; SQL rows could also be changed after staging because activation checked only counts/embeddings. | Implemented; focused portable and real-Postgres proofs green; final gate and independent re-review pending | The compiler contract owns one canonical serializer and validator, including platform-independent POSIX source-path, profile/resource, raw-source digest-shape, and projection checks. Lane 1 admission exact-binds the durable normalized bytes; Lane 2's fixed-role curator attests its raw-source digest as part of the complete generation. Compilation, source admission, staging, and activation recompute permission/projection/aggregate identities. Activation reconstructs the persisted non-embedding generation and revalidates its durable source admission under the exclusive tenant lock, rejecting every tested admission/policy/audience/concept/chunk/relationship mutation while preserving the prior active generation. |
...
| ARCH-04 | P2 | Architecture/human maintainability | Test ownership/cohesion | One 700+ line module mixed aggregate-gate contracts with owned Postgres/Docker lifecycle tests and fakes. | Implemented; exact current membership frozen | Gate contracts and owned Postgres runtime tests now live in separate functional modules. The checkpoint portable contract lists 24 modules/130 tests; the database contract lists 4 modules/17 tests. |
