...
physical lines: 274 at or above 350 and 238 from 250 through 349. The completed
...
mutually exclusive rows below classify every one of the 274
...
| Other `server/src/yap_server/**` | 3 | Retain the Scribe structured-edit contract, bounded model adapter, and asynchronous service as separate semantic, transport, and lifecycle owners. Admission, terminology, HTTP, and publication remain outside them; combining these state transitions would obscure validation or containment. |
...
| Other `server/tests/**` | 3 | Retain the Scribe semantic-validator, bounded model-adapter, and asynchronous-service suites beside their production owners. They cover adversarial edit integrity, masked source projection, and cancellation/containment separately and own no product state. |
...
| `server/src/yap_server/evaluation/transcript_correction_qualification_gate.py` | 622 | One exact-head aggregate composes source-evidence admission, public contracts, Rust/desktop/server children, warm-route qualification, teardown, and create-once public-safe publication. It does not own model/service mutation. |
...
| `server/tests/agents/test_transcript_correction_model.py` | 513 | One adapter suite covers strict response shape/bounds, request binding, protected masking/restoration, no-op removal, shortest unique context, and failure behavior without duplicating the semantic validator. |
