...
| `server/src/yap_server/evaluation/agent_service_lifecycle_observation.py` | 374 | One read-only observation owner validates exact service state, container launch policy, model readiness, process/listener absence, and the public-safe receipt projection without owning lifecycle mutation. |
| `server/src/yap_server/evaluation/agent_service_lifecycle_runtime.py` | 562 | One sequential lifecycle state machine stages the immutable launcher, owns the route network/supervisor/container observations, forces one restart, and proves every observed process and resource absent on success or failure. Splitting mutation from containment would weaken exact teardown. |
