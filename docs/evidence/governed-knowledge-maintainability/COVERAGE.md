...
the current staged Phase 10 Curator/profile-capacity candidate. That candidate
has public portable and structural evidence only; this document does not claim
a private Curator/route/workflow outcome, hosted review, merge, sustained
capacity, simultaneous full-profile residency, or production promotion.
...
At the current staged Phase 10 Curator/profile-capacity candidate tree, the
...
owners without relabeling them privately qualified. The mutually exclusive rows
below classify every one of the 297
...
| `server/src/yap_server/knowledge/knowledge_proposals.py` | 424 | One Postgres proposal owner validates strict citations, publishes immutable noncanonical proposals inside the caller-owned transaction, serializes exact per-subject unresolved capacity under an advisory lock, preserves idempotent retry, owns authorized discard/release, and coordinates proposal liveness with generation pruning. Canonical activation remains outside it. |
...
| `server/src/yap_server/evaluation/governed_knowledge_gate.py` | 573 | One aggregate decision composes exact candidate admission, the fixed 171-test portable membership, Ruff/Postgres/restart children, teardown, and create-once publication. Child lifecycle and evidence validators remain separate modules; current public execution of its portable membership is not a new private checkpoint outcome. |
...
| `server/tests/evaluation/test_agent_model_fixture_runner.py` | 791 | One conversation-driver test owner covers one-candidate requests, route and proposal output caps, tool/result sequencing, exact cited-proposal and semantic context withholding, warmups, contract parity, malformed tool-response continuation, and complex no-replay behavior. |
