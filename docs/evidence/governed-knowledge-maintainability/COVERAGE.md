...
| `server/src/yap_server/agents/transcript_correction_model.py` | 433 | One bounded model adapter owns protected-source masking, exact request binding, structured response parsing, no-op removal, and shortest unique edit-context normalization before the independent semantic validator. It does not own scheduling, persistence, or publication. |
...
| `server/tests/agents/test_transcript_correction_model.py` | 506 | One adapter suite covers strict response shape/bounds, request binding, protected masking/restoration, no-op removal, shortest unique context, and failure behavior without duplicating the semantic validator. |
