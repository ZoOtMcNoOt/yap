...
mutually exclusive rows below classify every one of the 253
...
| `server/tests/evaluation/test_agent_model_qualification.py` | 748 | One fail-closed decision test owner covers full admission, evidence-schema rejection, route-specific runtime/failure, protected-build-input tamper, common/proposal latency policy, and exceptional containment cases against the same qualification seam. |
