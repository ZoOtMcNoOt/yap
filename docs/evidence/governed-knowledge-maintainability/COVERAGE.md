...
| `server/src/yap_server/evaluation/agent_service_lifecycle_observation.py` | 373 | One read-only observation owner validates exact service state, container launch policy, model readiness, process/listener absence, and the public-safe receipt projection without owning lifecycle mutation. |
