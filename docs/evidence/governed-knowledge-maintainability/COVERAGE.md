...
At the current Phase 10 Student candidate tree, the inventory contains 526
in-scope tracked source, text, policy, and provenance surfaces at or above 250
physical lines: 285 at or above 350 and 241 from 250 through 349. The completed
...
mutually exclusive rows below classify every one of the 285
...
| Current/normative `docs/**` excluding completed/archived plans and research | 18 | Retain each ADR, current architecture/status, active decision queue, runbook, spec, and Voice OS document by taxonomy/decision owner. Stale current-state claims are repaired; these files are not executable modules. |
...
| `server/src/yap_server/evaluation/**` | 39 | Retain each named acceptance, corpus/review, scorer, runtime observation, lifecycle, qualification, and aggregate-decision owner. Scribe and Student qualification/gate modules separate private input admission, measurement, and aggregate lifecycle rather than sharing runtime mutation. The existing agent qualification and supervised-service lifecycle owners remain separate for the same reason. |
...
| Other `server/src/yap_server/**` | 7 | Retain Archivist ingestion plus the Scribe and Student semantic, model, and lifecycle owners as separate source, validation, transport, and workflow boundaries. Admission, terminology, HTTP, and publication remain outside them; combining these state transitions would obscure validation or containment. |
...
| Other `server/tests/**` | 5 | Retain the Scribe and Student semantic/model/lifecycle suites beside their production owners. They cover adversarial edit or premise integrity, source projection, permission-safe PostgreSQL reads, and cancellation/containment separately and own no product state. |
...
| `server/src/yap_server/agents/transcript_correction_model.py` | 439 | One bounded model adapter owns protected-source masking, exact request binding, structured response parsing, no-op removal, protected-value restoration, and shortest raw whole-token edit-context normalization before the independent semantic validator. It does not own scheduling, persistence, or publication. |
...
| `server/src/yap_server/evaluation/transcript_correction_qualification_gate.py` | 422 | One exact-head aggregate composes source-evidence admission, public contracts, Rust/desktop/server children, warm-route qualification, teardown, and create-once public-safe publication. It does not own model/service mutation. |
...
| `server/tests/agents/test_transcript_correction_model.py` | 562 | One adapter suite covers strict response shape/bounds, request binding, protected masking/restoration, no-op removal, shortest unique context, and failure behavior without duplicating the semantic validator. |
...
| `server/src/yap_server/agents/student_model.py` | 559 | One bounded Student model contract owns the strict JSON schema, exact visible-citation selection, unique support-quote resolution, exact source-subject boundaries, canonical support rebinding, server-derived spans, and deterministic question rendering. It owns no database, lease, or product publication state. |
...
| `server/src/yap_server/evaluation/student_qualification.py` | 640 | One Student decision synchronizes distinct owners, recomputes term/support/grounding/latency checks, observes the unchanged warm provider and broker, and publishes no question text in public evidence. The aggregate gate owns database/runtime orchestration. |
| `server/src/yap_server/evaluation/student_qualification_gate.py` | 789 | One exact-head Student aggregate owns full-profile admission, temporary reviewed knowledge, real PostgreSQL restart/cross-owner/audit verification, warm provider/broker observations, exact teardown, and create-once private publication. It never launches, swaps, or reduces the Qwen service. |
| `server/tests/agents/test_student.py` | 870 | One Student contract/lifecycle suite covers request/evidence identity, hidden-source rejection, exact subject boundaries, canonical support rebinding, unique support spans, queueing, cancellation, containment, and terminal audit without duplicating PostgreSQL integration. |
...
| `server/src/yap_server/evaluation/agent_model_fixture_runner.py` | 539 | Conversation sequencing, route-specific proposal output bounds, tool/result rounds, and bounded final structural decoding remain one evaluation driver after the duplicate product-tool schema authority was removed. Completed tools sit outside the retry loop. |
...
| `server/tests/evaluation/test_agent_model_fixture_runner.py` | 790 | One conversation-driver test owner covers route and proposal output caps, tool/result sequencing, exact cited-proposal and semantic context withholding, warmups, contract parity, malformed tool-response continuation, and complex no-replay behavior. |
