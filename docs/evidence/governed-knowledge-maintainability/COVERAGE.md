...
At the current Phase 10 Student candidate tree, the inventory contains 527
in-scope tracked source, text, policy, and provenance surfaces at or above 250
physical lines: 287 at or above 350 and 240 from 250 through 349. The completed
...
mutually exclusive rows below classify every one of the 287
...
| `server/tests/evaluation/**` | 14 | Retain one suite per corpus/runtime/qualification/evidence owner. Aggregate-gate and owned-Postgres lifecycle tests stay split; Scribe and Student qualification each retain an independent multi-owner acceptance owner. |
...
| `server/src/yap_server/agents/student_model.py` | 512 | One bounded Student model contract exposes ordered evidence text, accepts exactly one evidence index, source subject, and support quote, requires an exact subject-inside-quote-inside-evidence chain without topic copying, binds the selected canonical evidence and server-owned citation identity, derives the support span, enforces exact source boundaries, and renders the deterministic question. It owns no database, lease, or product publication state. |
...
| `server/src/yap_server/evaluation/student_qualification_gate.py` | 812 | One exact-head Student aggregate owns full-profile admission, ordered multi-item compiled evidence, temporary reviewed knowledge, per-case evidence-read counts, real PostgreSQL restart/cross-owner/audit verification, warm provider/broker observations, exact teardown, and create-once private publication. It never launches, swaps, or reduces the Qwen service. |
| `server/tests/agents/test_student.py` | 841 | One Student contract/lifecycle suite covers exact-one response shape, prompt grounding, index bounds, request/evidence identity, hidden-source rejection, exact Unicode and numeric subject boundaries, server-owned citation binding, queueing, cancellation, containment, and terminal audit without duplicating PostgreSQL integration. |
