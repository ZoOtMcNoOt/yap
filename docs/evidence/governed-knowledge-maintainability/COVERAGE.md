...
| `server/tests/agents/test_curator.py` | 1,024 | One Curator semantic/lifecycle suite covers both explicit triggers, exact Student lineage and citations, one forced model decision, publish-once behavior, rejection/invalid-output no-publish behavior, durable replay, queueing, deadlines, cancellation races, capacity failure, terminal audit, and containment without duplicating real-PostgreSQL orchestration. |
