...
through exact Student product candidate `778a7545...`. Its authenticated
HTTP/server boundary is privately qualified; the native/renderer path is
exact-head public-test green; hosted review and merge remain pending. The
...
At the Student-product documentation-successor tree, the inventory contains 610
in-scope tracked source, text, policy, and provenance surfaces at or above 250
physical lines: 359 at or above 350 and 251 from 250 through 349. The completed
checkpoint reviews remain the authority for their historical exact heads. This
freeze-time public read-back adds the Student authenticated HTTP runner, native
...
| `desktop/src-tauri/src/**` | 90 | Retain along existing functional module boundaries: app activation, audio session/coordinator, commands, job drain/ledger/remote state, language/live runtime, connector/auth, model adapters, trusted-source/correction revision, Librarian query/HTTP, Archivist ingestion/HTTP, and Student request/HTTP owners each own one state machine or transaction family. Inline tests remain beside their private Rust owners. The product verticals add no second credential or knowledge authority. |
...
| `server/src/yap_server/evaluation/**` | 54 | Retain each named acceptance, corpus/review, scorer, runtime observation, lifecycle, qualification, and aggregate-decision owner. Librarian, Analyst, Coordinator, and Auditor keep their deterministic evaluation and exact-head gate owners separate; the Librarian, Archivist, and Student product gates separately own their authenticated HTTP composition proofs. Public decisions, database lifecycle, broker probes, and publication boundaries do not become request-time owners. |
...
| Other `server/src/yap_server/**` | 25 | Retain the HTTP application plus Archivist and the eight role semantic/lifecycle/audit owners as separate routing, validation, transport, persistence, and workflow boundaries. Librarian query, Archivist ingestion, and Student question jobs compose existing cores without moving identity, evidence, recording-result, question, or knowledge authority into HTTP. |
...
| `server/tests/contract/contract_http_values.py` | 480 | One public HTTP contract registry enumerates operation IDs, capability fields, examples, runtime modes, schema names, and route paths for generated/spec drift detection; it owns no request execution. |
| `server/tests/contract/test_examples_contract.py` | 372 | One OpenAPI/example contract suite proves capabilities, errors, examples, and schemas stay mutually consistent, including the Archivist feature flag and strict request/result shapes. |
| `server/tests/jobs/test_runtime.py` | 1,052 | One runtime-composition suite spans the existing server modes and their fail-closed configuration matrices; service semantics remain in their dedicated owners and the suite creates no runtime fallback. |
...
| `server/src/yap_server/api/app.py` | 423 | One HTTP application composition root installs authenticated job, correction, capability, Librarian query, Archivist ingestion, and Student question routes and owns startup/shutdown ordering; each protocol handler and runtime remains in its functional module. |
