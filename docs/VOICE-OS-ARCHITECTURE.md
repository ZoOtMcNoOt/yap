...
| Transcript correction | Renderer/Ollama Polish is removed. The Scribe candidate uses authenticated native/server finalized-segment correction, visible diff, and a separate accepted revision; private qualification/merge are pending. | Complete Scribe plus the remaining bounded agents through governed already-warm Qwen/Gemma routes |
...
| LLM | No shipped local LLM; raw ASR remains the correction fallback | Supervised Qwen/Gemma profiles and owner-fair admission are merged; the Scribe consumer is implemented but unqualified/unmerged, and sustained multi-user capacity remains open |
...
flowchart TB
    ...
    subgraph L2Batch["L2 batch — server path · Yap recording quality"]
        ...
        ScribeB["Scribe candidate<br/>authenticated finalized correction · rapid route"]
    end
...
| **L2** Post-LLM / transcript correction | ✅ Reconciled | ADR 0005, ADR 0031 | The current Scribe candidate uses the authenticated already-warm Qwen rapid route, publishes only a manually accepted source-bound revision, and preserves raw ASR on failure. Any solo/local `llama-server` product remains separately deferred and unqualified; Ollama Polish is deleted. |
...
alignment/result publication and the current Scribe candidate owns only
...
| **10** | Slices 10.1–10.3 merged; Scribe candidate implemented and public-green | The merged baseline implements one systemd-contained Rust provider-launcher lifecycle, immutable Qwen/Gemma profiles, and bounded owner-fair admission for all eight role bindings over an owner-private Rust/Python boundary. PRs #155/#157/#158 merged those layers; the admission merge is `84d95842...`. The current Scribe candidate is the first authenticated consumer and preserves raw ASR while publishing only a user-accepted correction revision. Its private qualification, aggregate gate, and hosted merge are pending. The other seven roles, warm simultaneous residency, mixed-load capacity promotion, observability, corporate access hardening, HTTP/3 edge promotion, publication governance, and repo split remain later slices or IT handoffs. |
...
`84d95842...`. The Scribe product workflow is implemented and public-green;
its bilingual/multi-owner private qualification, aggregate gate, hosted merge,
and the remaining seven roles are next.
Live ASR,
...
| **10** Enterprise/release | Slices 10.1–10.3 merged the systemd/Rust provider lifecycle, exact Qwen/Gemma profiles, and authenticated already-warm owner-fair admission. Scribe is the first implemented consumer; private qualification/merge and seven roles remain before simultaneous residency, sustained mixed-load capacity promotion, observability, enterprise access, secure-edge evidence, packaging, and repo split. | [ADRs 0030](adr/0030-rust-supervised-provider-service-lifecycle.md) and [0031](adr/0031-eight-agent-voice-os-roster.md), the active Phase 10/roster plans, merged PRs #155/#157/#158, Scribe exact-head evidence when consumed, later route-specific capacity/SLO evidence, CI/CD migration, and cross-repo link update. |
