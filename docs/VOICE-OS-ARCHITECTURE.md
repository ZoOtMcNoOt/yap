...
`97b63be46b05dffa21595f2fd081b8467bb95798`, while comparative behavior and
...
`97b63be46b05dffa21595f2fd081b8467bb95798`; natural short-correction quality
...
`97b63be46b05dffa21595f2fd081b8467bb95798` passed the Phase 6 lifecycle,
...
`97b63be46b05dffa21595f2fd081b8467bb95798`; neither has been promoted as a
...
| **6** | Bounded final repair review passed; replacement gate pending | ADRs 0024–0026 and the active plan define the provider catalog, primary language, bounded resident AmberNet/Nemotron Preview, verify-only five-region AmberNet batch preflight, explicit server Nemotron auto mode, fail-closed alignment, and provider-specific serving gates. Exact executable candidate `97b63be46b05dffa21595f2fd081b8467bb95798` passed its frozen 30-child local/native/server/private-runtime matrix, but the required final adversarial review found concrete restart/cleanup, normative OpenAPI, hosted-closure, evidence-bound, and persisted-vocabulary defects and invalidated that SHA as merge authority. Its receipt remains historical evidence. Admitted replacement `4b87e222c8ad7325a12a88709a52b5e9c1baf22e` failed before provider startup when its build forced a registry lookup across the offline GB10 boundary; exact cleanup passed and that SHA cannot be retried. The current checked-runtime and containment repair passed one bounded final three-agent re-review. Runtime images are now prepared before admission and emit private receipts after a second clean-head check. Admitted gates verify each frozen receipt hash and exact prepared ARM64 image identity, launch the receipt-bound immutable ID, and bind it into evidence; they cannot build, pull, reconnect, or substitute an image. One new complete gate remains. The local route remains default-off Preview because its natural-switch target failed; the catalog still advertises only gated Cohere `en-US` with `wordAlignment: false`; neither resident server provider is promoted. Hosted checks, PR review, and merge remain open. Tiron/provider quality selection stays in Phase 8; authentication and persistent supervised mixed-load production remain Phases 7 and 10. |
...
**Next execution order:** finish the Phase 6 executable repairs and adversarial
read-back, freeze one replacement candidate, and run its complete
local/native/server/GB10 gate exactly once. Then run hosted checks on the
focused PR head, review, and merge. Run the separately queued Checkpoint B
before continuing Phases 7–10 on separate branches in documented order. WSS/live
...
- [x] Complete the digest-pinned Cohere vLLM candidate-safety gate with frozen
  ...
  `97b63be46b05dffa21595f2fd081b8467bb95798`; retain output determinism,
...
- [x] Complete Nemotron NeMo's independent request-lifecycle, fixed/auto language-
  ...
  `97b63be46b05dffa21595f2fd081b8467bb95798`; retain provider-behavior quality,
