...
`284ab96b...`. Curator is not product-exposed. Librarian, Analyst, Coordinator,
and Auditor remain absent. The
...
| **Eight bounded role workflows** | ✅ Reconciled | [ADR 0031](adr/0031-eight-agent-voice-os-roster.md) | Shared authenticated bounded admission and four role cores—Scribe, Archivist, Student, and Curator—are merged through PR #168. Librarian, Analyst, Coordinator, and Auditor remain. Historical one-slot evidence and local-mutex context remain scoped to their exact heads. |
...
| **10** | Four role cores merged; four workflows remain | The merged baseline implements one systemd-contained Rust provider-launcher lifecycle, immutable Qwen/Gemma profiles, bounded owner-fair admission, qualified Scribe, the no-LLM Archivist core, and the internal Student and Curator cores. PR #164 merged Scribe as `ec3af506...`; PR #165 merged Archivist as `2a7ec819...`; PR #166 merged Student as `2254605e...`. Exact route head `dab19fe...` returned `required-workload-routes-qualified`; exact workflow head `7cd24deb...` requalified Scribe/Student and returned `curator-knowledge-proposals-qualified`; exact public-lock/aggregate head `7f896b34...` returned `governed-knowledge-gate-passed`. Hosted-green head `593e627b...` passed all 12 checks, and PR #168 merged the Curator/profile-capacity slice as `284ab96b...`. Raw ASR remains authoritative and Curator may append only a noncanonical proposal. Student/Curator product integration, the four remaining workflows (Librarian, Analyst, Coordinator, and Auditor), a second full-strength GPU node/private route for simultaneous residency, sustained mixed-load capacity promotion, observability, corporate access hardening, HTTP/3 edge promotion, publication governance, and repo split remain later slices or IT handoffs. |
...
Student/Curator product integration remains open. Librarian, Analyst,
Coordinator, and Auditor are next, alongside the separate full-strength
