...
and authenticated REST/private-WebSocket admission exist; live ASR, a bound
provider profile, and the managed enterprise edge remain absent. Merged ADR
0030 Slice 10.1 implements only the hardware-independent
...
evaluation capabilities. Rust now implements the merged generic lifecycle
baseline, but neither qualified route is bound to it. The vLLM
...
| **10** | Slice 10.1 merged; Slice 10.2 next | The merged hardware-independent baseline implements one systemd-contained, Rust-owned foreground provider-launcher lifecycle with exact loopback/model readiness, bounded restart/stop, and private state counters. Exact hosted-green head `1a487db8...` merged through PR #155 as `e2d82b89...`. Qualified provider profiles, application integration, simultaneous residency, warm/multi-worker and mixed-load capacity promotion, observability, corporate access hardening, HTTP/3 edge promotion, publication governance, and repo split remain later slices or IT handoffs. |
...
| **10** Enterprise/release | Merged Slice 10.1 begins with one systemd-contained Rust provider-launcher lifecycle; later slices add qualified service profiles, authenticated routing, simultaneous residency, warm/multi-worker and mixed-load capacity promotion, observability, Zscaler/corp access, HTTP/3 secure-edge benchmark/promotion, packaging, and repo split | [ADR 0030](adr/0030-rust-supervised-provider-service-lifecycle.md), the active Phase 10 plan, and merged PR #155 evidence; later route-specific capacity/SLO evidence; CI/CD migration; cross-repo link update |
