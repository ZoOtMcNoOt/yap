...
| **6** | Active; catalog slice implemented, phase not gated | ADR 0024 and the active Phase 6 plan define the provider catalog, primary language, guarded SpeechBrain preflight, explicit server Nemotron auto mode, and fail-closed alignment path. A bounded catalog now joins immutable model identity only after artifact verification, serves separately from health, and passes native fingerprint validation; it advertises only the gated Cohere `en-US` fixed-batch route. Primary-language UX/state, preprocessing, LID, Nemotron dynamic routing, alignment, durable stage evolution, representative locale gates, and the complete Phase 6 matrix remain unfinished. |
