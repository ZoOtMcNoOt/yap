...
snapshot last reconciled 2026-08-11
...
and authenticated REST/private-WebSocket admission exist; live ASR, merged
persistent supervision, and the managed enterprise edge remain absent. The
unmerged ADR 0030 Slice 10.1 candidate now implements only the hardware-independent
one-launcher lifecycle boundary. The accepted performance topology is provider-specific: Cohere batch
...
evaluation capabilities. Rust now implements the unmerged generic lifecycle
candidate, but neither qualified route is bound to it. The vLLM
...
| **10** | Active Slice 10.1 candidate | The unmerged hardware-independent candidate implements one systemd-contained, Rust-owned foreground provider-launcher lifecycle with exact loopback/model readiness, bounded restart/stop, and private state counters. Hosted Linux proof and merge remain open. Qualified provider profiles, application integration, simultaneous residency, warm/multi-worker and mixed-load capacity promotion, observability, corporate access hardening, HTTP/3 edge promotion, publication governance, and repo split remain later slices or IT handoffs. |
...
`fc8a1651...`. Phase 10 Slice 10.1 is an active unmerged candidate. It must pass
its exact-head hosted Linux/repository checks and merge before provider-specific
Slice 10.2 begins.
Live ASR,
...
| **10** Enterprise/release | Slice 10.1 begins with one systemd-contained Rust provider-launcher lifecycle; later slices add qualified service profiles, authenticated routing, simultaneous residency, warm/multi-worker and mixed-load capacity promotion, observability, Zscaler/corp access, HTTP/3 secure-edge benchmark/promotion, packaging, and repo split | [ADR 0030](adr/0030-rust-supervised-provider-service-lifecycle.md) and the active Phase 10 plan; hosted Slice 10.1 evidence/merge; later route-specific capacity/SLO evidence; CI/CD migration; cross-repo link update |
