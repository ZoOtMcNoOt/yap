...
| Live language | Executing fallback applies the confirmed primary locale across Nemotron's exact 32 out-of-box locales. The Phase 6 branch now implements one optional bounded resident acoustic-LID component, offline switching, and within-utterance source-time language spans under the same Rust live-runtime owner. Focused tests plus eight- and four-logical-CPU paced i7 development-host profiles pass. Fail-closed checked-head repeated-session and release-mode default-microphone/UI collectors bind the observed Windows processor and processor count but remain unconsumed on the final checked head. The proportional MVP gate uses a direct local runtime with no server client plus an isolated UI profile with no configured or listening numeric-loopback server. Prepared audio supplies speech/routing/transcription evidence; a 30-second unattended UI smoke supplies capture lifecycle, responsiveness, save/delete, and teardown evidence without assuming Windows output reaches the selected microphone. The consumed representative natural-switch target failed, so the feature remains explicit, default-off Preview behavior while current-host resource/lifecycle closure remains open. Longer manual physical-device and representative low-end battery/thermal certification is deferred to default-on or Phase 10 release qualification. | Multilingual server live follows the authenticated live-transport gate. Local switching remains a best-effort Preview unless new independently frozen evidence justifies a stronger quality tier. |
...
A checked wrapper now composes their plan-owned lifecycle
cells sequentially and removes its temporary bridge before final evidence; that
wrapper is executable under focused tests but has not run on the frozen GB10
head. The NeMo implementation currently
...
the model-neutral candidate-safety GB10 lifecycle passed at exact head
`a21964c19e56648e9fddcb5200de419e59a7687c`, while comparative behavior and
...
therefore `desktop-prepared-audio-frame-to-final`; current-host physical
microphone, rendered-UI/resource, and natural short-correction evidence remain
...
| **L2** Batch LangID | ✅ Reconciled | ADR 0003/0008/0024/0026 | **Off the live hot path**; the Phase 6 branch verifies one imported AmberNet 1.12.0 INT8 QDQ artifact and samples five strict six-second regions from start through exact tail before explicit user confirmation. Focused real-model/parity/contract tests exist, and exact head `a21964c19e56648e9fddcb5200de419e59a7687c` passed the final source-exact ARM64 production-worker resource/lifecycle repetition. Representative suggestion quality and complete phase evidence remain open. The prior SpeechBrain receipt is historical. |
...
model-neutral candidate-safety lifecycle passed at exact head
`a21964c19e56648e9fddcb5200de419e59a7687c`; neither has been promoted as a
...
| **6** | Active; catalog/language/durable-stage/VAD/local-span/server-runtime slices implemented, phase not gated | ADRs 0024–0026 and the active plan define the provider catalog, primary language, one bounded resident local acoustic-LID/span engine, verify-only five-region AmberNet batch preflight, explicit server Nemotron auto mode, fail-closed alignment, and provider-specific serving gates. Rust-owned fixed decisions, the visible catalog-derived recording-language selector, durable stages, advisory Silero VAD, one-owner local AmberNet/Nemotron switching, source-time spans, strict user-confirmed batch suggestions, pinned reference routes, Cohere vLLM and Nemotron NeMo candidates, and correction/history contracts execute under focused tests. The local automatic route remains an explicit default-off Preview because its frozen natural-switch target failed. Checked-head native repeated-session and release-mode microphone/rendered-UI collectors now bind the observed Windows processor and processor count, but they are unconsumed on the final checked head. The revised proportional gate requires a direct local runtime with no server client and an isolated UI profile with no configured or listening numeric-loopback server. Prepared audio owns speech/routing/transcription evidence, and one unattended 30-second UI smoke owns capture lifecycle, responsiveness, save/delete, and teardown without assuming Windows output reaches the selected microphone; longer manual physical-device and low-end battery/thermal certification is deferred to default-on or Phase 10 release qualification. The server batch preflight independently samples source start through exact tail and abstains unless five positive-margin AmberNet regions agree; focused real-model and AMD64/ARM64 parity evidence exist, and exact head `a21964c19e56648e9fddcb5200de419e59a7687c` passed the final source-exact ARM64 production-worker repetition, while the old SpeechBrain receipt is historical. The shipped catalog intentionally advertises only gated Cohere `en-US` with `wordAlignment: false`; Phase 6 does not manufacture a second Cohere locale solely to populate the selector. The same exact GB10 head passed the 18-child Cohere vLLM/Nemotron NeMo candidate-safety lifecycle with exact teardown, while leaving both routes unpromoted and broad quality/replacement work in Phase 8. Exact executable head `05d1b82017447df04d46ccc8fa729c5a6a0d0b13` also passed the focused connected Windows-desktop/SSH-forwarded-GB10 route through durable preprocessing, one tunnel interruption, server-authoritative result publication, verified History review, and complete remote teardown. The automated current-host Preview lifecycle, complete accessibility rerun, and frozen complete Phase 6 matrix remain unfinished. Removing Preview requires new independent natural/noisy evidence. Alignment remains intentionally unavailable until a later selected route passes its own promotion gate. Phase 8 evaluates whether Tiron replaces any Cohere batch workload and owns broad Cohere quality, long-duration, percentile, and rollback work; Phase 6 does not claim server live, authenticated admission, or a persistent supervised mixed-load production service. |
...
- [x] Isolated verify-only CPU AmberNet long-recording suggestion with five
  ...
  resource/lifecycle repetition at
  `a21964c19e56648e9fddcb5200de419e59a7687c`
...
- [x] Complete the digest-pinned Cohere vLLM candidate-safety gate with frozen
  ...
  admission, and teardown evidence at
  `a21964c19e56648e9fddcb5200de419e59a7687c`; retain output determinism,
...
- [x] Complete Nemotron NeMo's independent request-lifecycle, fixed/auto language-
  ...
  admission, recovery, and lifecycle gate at
  `a21964c19e56648e9fddcb5200de419e59a7687c`; retain provider-behavior quality,
