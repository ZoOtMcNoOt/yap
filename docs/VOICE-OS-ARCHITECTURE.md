...
| **Graceful degradation** | Revisioned Scribe, typed workflow failures, governed evidence gates, and raw/local controls when the server is unavailable — production-minded. |
...
| LLM | No shipped local LLM; raw ASR remains the correction fallback | Supervised Qwen/Gemma profiles, owner-fair admission, and all eight bounded role cores are merged. The complex route is batch invariant with prefix caching disabled. Simultaneous full-profile residency needs a second owned GPU node/private routing, and sustained multi-user capacity remains open. |
...
flowchart TB
    ...
    subgraph Yap["Yap — Tauri + React"]
        UI["Transcribe · Live · History<br/>Scribe manual correction · governed KB baseline"]
    end

    Orch["Domain-owned orchestration<br/>Nemotron + server health/batch state now<br/>server LLM admission current · live WSS deferred"]
    ...
    UI --> Batch
    Live --> CR
    UI -->|manual Scribe correction| LL
    Live --> Handoff
    Batch --> Handoff
    Handoff -.-> KW
    KW -.-> L3
...
`284ab96b...`. Exact `6aa33e4d...` privately qualified Curator's product server
boundary with 10/10 exact terminals and public-safe evidence SHA-256
`328f6640...`; hosted head `b983adb7...` passed all 12 checks and PR #179 merged
the vertical as `70303872...`. Exact
`56b7f5d0...` privately
...
cited proposal-bundle core; hosted head `53ee0152...` passed all 12 checks and
PR #171 merged it as `67d836da...`. Exact `08b06f6d...` privately qualified
Auditor's idle-only source-cited review-findings core; hosted head
`937a4129...` passed all 12 checks and PR #172 merged it as `1b255e9a...`.
...
qualified the Archivist authenticated staging boundary with 10/10
...
head `6890a9f5...` passed all 12 checks and PR #181 merged its HTTP/native/
renderer successor as `3fd5eaed...`. Exact `87924d5f...` privately qualified
Auditor's product server/database/broker boundary with 10/10 exact terminals and
public-safe evidence SHA-256 `b5a31c21...`; hosted head `6bb72953...` passed all
six CI jobs reported for PR #183, which rebase-merged the tree-identical HTTP/
native/renderer successor with main tip `13d9e3ef...`.
The
...
flowchart TB
    ...
    subgraph Server["yap-server - GB-class node (org LAN/VPN)"]
        ...
        LLM["TARGET vLLM agent/LLM plane\nroute-specific cache policy + structured outputs"]
    ...
    LLM -->|"bounded correction / agent response"| Router
    Router --> KB
...
flowchart TB
    ...
    subgraph L1["L1 — OS listeners + pre-warm · client-owned"]
        ...
        PreWarm["Warm Nemotron + mic now<br/>team LLM service is server-owned · live Silero deferred"]
    end
    ...
    subgraph L3["L3 — meeting evidence, review, and ingestion · Phase 8/9 substrate + Phase 10 Archivist"]
        ...
        Review["Durable reviewed source admission"]
        Archivist["Archivist · OKF markdown + YAML · no LLM"]
        IngestFailure["Typed ingestion failure · source preserved"]
    end
    ...
    subgraph L5["L5 — governed agent workflows · Phase 9 substrate + Phase 10 roster"]
        Student["Student · exact-source learning questions"]
        Curator["Curator · governed proposal only"]
        Auditor["Auditor · source-cited findings · IDLE_ONLY"]
    end
    ...
    Align --> Intersect
    Diar --> Intersect
    Intersect --> Stitch --> Review --> Archivist --> KB
    Archivist --> IngestFailure
    KB --> Conv & Gloss & Art & Team & Media
    Stitch --> History

    Conv --> Student
    Student -.->|explicit reviewed answer or proposal| Curator
    Conv --> Auditor
    KB --> MCP & IDE & Vec
...
| **L5** Student and Curator | ✅ Reconciled | ADR 0031 | Student produces bounded source-subject questions; Curator may publish governed proposals only. No notification blacklist or direct git mutation. |
| **L5** Auditor | ✅ Reconciled | ADR 0031 | Current internal core accepts explicit authenticated manual requests under idle-only admission and returns source-cited review findings; no mutation or scheduled autonomy. |
...
| **Eight bounded role workflows** | ✅ Reconciled | [ADR 0031](adr/0031-eight-agent-voice-os-roster.md) | Shared authenticated bounded admission and all eight role cores—Scribe, Archivist, Student, Curator, Librarian, Analyst, Coordinator, and Auditor—are merged through PR #172. Historical one-slot and pre-batch-invariant evidence remain scoped to their exact heads. |
...
### Agent roster (8 bounded role workflows)

The current workflow, authority, route, output, and failure contract is
**[ADR 0031](adr/0031-eight-agent-voice-os-roster.md)**. Historical profile and
state-machine context remains in **[ADR 0006](adr/0006-silero-agents-state-machine.md)**.
...
| **Archivist** | Invalid source or write failure | Preserve the durable source and report a typed ingestion failure |
| **Student** | Invalid, unsupported, or unavailable evidence | Publish no questions; leave conversation unchanged |
| **Curator** | Invalid review, citation, or proposal | Reject; publish no proposal or success audit |
...
today. Entries explicitly marked deferred below are separately governed future
work, not compatibility obligations for the current team route.
...
| **10** | All eight bounded role cores and all eight authenticated product surfaces merged | The merged baseline includes all eight internal cores through PR #172 and all eight product surfaces through PR #183. Exact `87924d5f...` privately qualified Auditor's authenticated product server/database/broker boundary with 10/10 exact terminals, 4 exact server-derived noncanonical review-required reports, strict owner isolation, two PostgreSQL restart/read-backs, c8/ninth-owner-queued containment, and public-safe evidence SHA-256 `b5a31c21...`. Hosted head `6bb72953...` passed all six CI jobs reported for PR #183, which rebase-merged the tree-identical successor with main tip `13d9e3ef...`. Simultaneous residency, sustained capacity, observability, enterprise access, secure-edge promotion, publication governance, and repo split remain later slices or IT handoffs. |
...
`5e187ed4...`; hosted-green head `bc9a88bc...` merged through PR #164 as
`ec3af506...`. Hosted-green Archivist head `e1899db7...` merged through PR #165
as `2a7ec819...`. Exact Student predecessor `0970d74c...` ran 1,241 portable
tests (1,207 passed and 34 declared skips) on the unchanged full Qwen rapid
profile but returned terminal `deterministic-no-student` with public-safe
evidence SHA-256 `316631d5...` after seven of eight cases completed. The
...
server/database/broker boundary; hosted head `6890a9f5...` passed all 12 checks
and PR #181 merged it as `3fd5eaed...`. Exact `87924d5f...` privately qualified
Auditor's product server/database/broker boundary; hosted head `6bb72953...`
passed all six CI jobs reported for PR #183, which rebase-merged the tree-
identical product successor with main tip `13d9e3ef...`.
Coordinator
...
| **10** Enterprise/release | All eight bounded role cores are merged through PR #172, and all eight authenticated product surfaces are merged through PR #183. Exact `87924d5f...` privately qualified the Auditor server boundary with public-safe evidence SHA-256 `b5a31c21...`; hosted head `6bb72953...` passed all six reported PR jobs and rebase-merged tree-identically with main tip `13d9e3ef...`. Second-node simultaneous full-profile residency, sustained mixed-load capacity promotion, observability, enterprise access, secure-edge evidence, packaging, and repo split remain open. | [ADRs 0030](adr/0030-rust-supervised-provider-service-lifecycle.md) and [0031](adr/0031-eight-agent-voice-os-roster.md), the completed roster-delivery plan, the product evidence records through [Auditor](evidence/auditor-product-vertical/VERIFICATION.md), route-specific sustained capacity/SLO evidence, CI/CD migration, and cross-repo link update. |
...
The former bundled `llama-server`/renderer Polish checklist is not an active
team implementation path. Team Scribe and the bounded agents use authenticated,
server-owned Qwen/Gemma services. Any future solo/local LLM product must be a
newly qualified vertical slice; it must not restore the deleted renderer path or
act as a compatibility fallback.
...
| Historical solo/local llama-server design; team route superseded | [0005](adr/0005-llama-server-agents.md) |
...
| [Historical local LLM sidecar](specs/local-llm-sidecar.md) | deferred solo/local study; not the team Scribe route |
