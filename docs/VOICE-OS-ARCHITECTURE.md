...
| **8** | Active candidate contract; qualification pending | ADR 0020, ADR 0027, and the source-aware design are canonical. Track/timeline/recording prerequisites execute. The pinned upstream whole-meeting Tiron worker has focused offline GB10 short/two-window evidence, and the explicit meeting-only profile composes receipt-bound startup, three-hour admission, hash-bound anonymous-speaker/transcript revisions, owner-scoped retrieval, native publication, and History projection in focused tests. A supported-launcher HTTP/native roundtrip, local anonymous model, larger-roster speaker-epoch reconciler, frozen scored messy-meeting result, purpose-authorized reconciliation/naming, complete Phase 8 gate, and production promotion remain open. |
