...
retrieval, native publication, and History projection. A real supported-launcher
HTTP/native roundtrip remains part of the Phase 8 gate. It is not the default or
a production-promoted route.
...
| Phase 8: meeting evidence | Active; composed candidate path | [ADR 0027](adr/0027-tiron-joint-speaker-attributed-meeting-transcription.md) selects the pinned upstream Tiron runtime's eight-window/eight-global route as the server development baseline, queues a separately gated speaker-epoch extension for larger speaking rosters, and retains local anonymous evidence plus an ASR-plus-diarization fallback. Exact model/runtime/ECAPA/container/dependency identities and the messy-meeting acceptance contract have executable validators. A digest-pinned ARM64 image and thin source-bound worker passed focused offline GB10 short and two-window smokes. Explicit meeting-only candidate startup now verifies both locked model roots and a receipt-bound checked image; Python server tests compose three-hour admission, restart-safe hash-bound transcript/speaker revisions, and owner-scoped retrieval with Rust native publication and History projection. A real supported-launcher HTTP/native roundtrip, the reconciler, scorer/private-holdout results, full Phase 8 gate, and production promotion remain open. |
