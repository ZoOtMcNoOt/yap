...
| Phase 7: identity/access | Active; hosted portability repair qualified | The [tenant-scoped identity and job authorization plan](plans/active/2026-07-25-tenant-scoped-identity-and-job-authorization.md) governs the branch. Provider-neutral OIDC/Entra policy, tenant-scoped ownership, purpose authorization, protected readiness, authenticated bounded private WebSocket admission, and the native lower handshake execute under focused tests. The desktop still has no selected production token adapter. Exact head `944673071804d8178776efa1d1e13651c87df6fb` passed its private and complete matrix but was consumed when GitHub rejected its first workflow during parsing. Workflow repair `cafbe307e7203e09050fdbe2eb080d5d84b65026` is preserved after fresh target-client qualification exposed bounded local-ASR pre-roll saturation. Runtime repair `32cf52891c277a4a3d47aa9fb3cab105ca58af98` passed focused verification, same-three review, 12 repeated resource sessions, nine prepared-audio boundaries, and the physical-microphone/rendered-UI lifecycle with zero drops; the first isolated scheduler-outlier attempt remains preserved as failed evidence. Exact documentation successor `dc6359162fb16909d38f410cdb75c2729d83972f` then passed its one complete private 25-cell matrix and independent receipt validation. All four CodeQL analyses passed, but hosted CI run `30574652702` exposed three portability defects: a workflow-parser dependency loaded before package installation, GitHub Windows temp artifacts had an owner different from the current runner identity, and two tests compared equivalent 8.3 and long paths as raw strings. NSIS was not dispatched after that failure. Reviewed repair `558fed05e0f959a28fbe4d92499bbe185b0532d6` loads workflow YAML only in workflow-reading tests, corrects a mismatched Windows owner with the protected private DACL, and compares existing test paths by canonical identity. Pre-admission preparation of exact descendant `c95cfe02a4a1df81dfc4aaed58ac15f61247c4f4` then exposed that an unnecessary same-owner rewrite could require elevation under ordinary development-root ACLs; it was never admitted and no phase matrix ran. Repair `a823b28...` writes the owner only on an exact SID mismatch while retaining the post-write owner/DACL verification. The three Windows ACL behaviors, all 16 applicable hosted-portability contracts, and the exact clean-tree admission fixture pass; the same three independent lenses report no P0–P2 finding. Fresh exact-tree closure and admission, one complete replacement matrix, exact-head hosted closure, and merge remain open. |
...
- Phase 7 candidates `134ec08002aeb1deca83547d511528b282966731`,
  ...
  Fresh exact-tree closure and admission, the one replacement phase gate,
  first-valid-attempt hosted PR closure, and merge remain open.
