...
| Phase 7: identity/access | Active; adversarial repairs focused-green, bounded re-review pending | The [tenant-scoped identity and job authorization plan](plans/active/2026-07-25-tenant-scoped-identity-and-job-authorization.md) governs the branch. Strict Entra Yap-API token validation, token-derived `(tid, oid)` principals, durable access disable/restore, owner-scoped job/LID/idempotency/artifact behavior, protected readiness, the bounded official MSAL.NET/WAM desktop adapter, tenant-specific account authority, schema-13 quarantine of ambiguous older authenticated bindings, OS-protected cache, and sensitive bearer injection execute under focused tests. Exactly three initial adversarial reviews completed; their accepted P1/P2 repairs passed focused server, native, frontend, dependency, and release-contract verification. Bounded closure from those same reviewers, real enterprise Entra/WAM policy conformance, the complete matrix, hosted closure, and merge remain open. |
...
1. Reconcile the ready Phase 7 implementation and documents, complete exactly
   three bounded adversarial reviews, and repair accepted findings with focused
   verification.
2. Freeze one exact candidate, run the one-time Phase 7 matrix, and merge only the green,
