...
default-microphone/rendered-UI resource and lifecycle check, whose capture is
one unattended 30-second smoke under an isolated no-server profile. The
prepared-audio matrix owns speech and transcription evidence; the UI smoke owns
capture lifecycle, responsiveness, save/delete, and teardown.
...
1. Run one bounded automated Windows session for the local fallback and
   default-off language-switching Preview with no configured or listening
   server; no Wi-Fi change or manual stimulus control is required.
2. Reconcile claims, freeze one exact Phase 6 candidate, and run the complete
