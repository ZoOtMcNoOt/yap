...
- Phase 6 has focused implementation slices on the current branch: a verified
  ...
  execute under focused Python 3.12/Rust/React tests, but the checked-head image
  has not run in the final GB10 gate. The server Nemotron fixed/automatic routes,
...
2. Preserve the completed source-authoritative normalization/VAD, durable-stage,
   and isolated SpeechBrain preflight slices while promoting or replacing the
   implemented bounded resident acoustic-LID candidate with representative
   span/resource evidence and exercising the checked-head LID image on GB10.
