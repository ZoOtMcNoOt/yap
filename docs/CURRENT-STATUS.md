...
| Phase 7: identity/access | Active; focused implementation green, phase gate pending | The [tenant-scoped identity and job authorization plan](plans/active/2026-07-25-tenant-scoped-identity-and-job-authorization.md) governs the branch. Strict Entra Yap-API token validation, token-derived `(tid, oid)` principals, SQLite-backed identity/revocation/purpose/audit records, owner-scoped job/LID/idempotency/artifact behavior, the bounded official MSAL.NET/WAM desktop adapter, OS-protected cache, sensitive bearer injection, and durable account-switch protection execute under focused tests. Real enterprise Entra/WAM policy conformance, the complete matrix, adversarial review, hosted closure, and merge remain open. |
