...
**Last gated checkpoint candidate:** exact executable head
...
validation, and all required CI and CodeQL jobs. Documentation-only successor
`e22368fcf90410228c6be8da3d69cd177e0b106c` did not close the checkpoint:
its first hosted frontend audit exposed high-severity
`GHSA-mwp4-54f8-5fhr` in the development-only WDIO/Puppeteer chain. The current
repair pins patched `ip-address` 10.3.1. Because that changes the lockfile, the
repair requires a fresh exact-head candidate gate and hosted closure; neither
the failed hosted run nor the earlier candidate can qualify it. The functionally named
...
requalifying the model. Replacement candidate qualification, exact-head hosted
...
| Meeting-transcription maintainability checkpoint | Late dependency repair; replacement candidate pending | The [ownership review](plans/active/2026-08-03-meeting-transcription-ownership-and-maintainability-review.md) keeps server result adapters independent from worker admission, accepts only current persisted contracts, excludes evaluation code from the request-time image, linearizes native publication/cancellation, and bounds selected History speaker detail. Exact candidate `fb0985e7c08cf0a0e69752afbe61e372cbfe76db` passed its build, exact-image lifecycle, single 18-child matrix, receipt validation, and required CI/CodeQL jobs. Documentation successor `e22368fc...` then failed the frontend audit on `GHSA-mwp4-54f8-5fhr`. The current lockfile pins patched development-only `ip-address` 10.3.1, so the [behavior-named gate](runbooks/meeting-transcription-maintainability-checkpoint.md) must qualify a fresh exact head before hosted closure and PR #143 merge. This does not change model-quality, capacity, or production-promotion qualification. Private evidence remains outside Git and hosted artifacts. |
...
lineage. It receives a fresh candidate gate rather than an exception.
...
repair must pass a fresh exact-head candidate gate after the late development-
tool security pin, followed by hosted closure and PR #143 merge before Phase 9.
The concise
...
1. Gate and merge the repaired
   [meeting-transcription ownership and maintainability review](plans/active/2026-08-03-meeting-transcription-ownership-and-maintainability-review.md),
   only after its fresh candidate matrix, receipt validation, and exact-head
   hosted closure including disposable-Windows NSIS pass.
