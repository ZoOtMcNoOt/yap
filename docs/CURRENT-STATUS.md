...
**Most recent merged closure:** [PR #180](https://github.com/mcnatg1/yap/pull/180)
merged the Analyst HTTP/native/renderer product vertical as
`c95fcf1a043f919661b007f014a8dc6729aa02f2` after exact hosted head
`4c8db7c22f6a8ebadbc73433155f060b5b699fb6` passed all 12 required checks.
PR #179 previously merged Curator, PR #178 merged Student, PR #177 merged Archivist,
and PR #174 merged Librarian. Earlier qualification,
...
Exact candidate `e2ba1864ec989a2bb693e000d0b4c30910d2720f` now implements
the first Librarian product vertical: authenticated asynchronous HTTP queries,
native-owned bearer transport and evidence validation, and a dedicated
Knowledge workspace with bounded citations, cancel/retry controls, and graceful
remote unavailability. Its owner-private database/broker/HTTP gate returned
`librarian-authenticated-product-server-boundary-qualified` with public-safe
evidence SHA-256 `45eac229...`: eight synchronized owners, 10/10 exact
terminals, strict bearer and cross-owner denial, exact durable audits, two
PostgreSQL restarts, and complete teardown. Public exact-head checks are also
green: server **1,521 = 1,474 passed + 47 declared skips**, governed fixed set
...
qualified the merged Archivist product vertical: authenticated asynchronous HTTP
staging, native-owned bearer/job/result transport, and one explicit **Stage for
knowledge** action for a completed server-batch transcript. Its owner-private
ARM64 database/broker/HTTP gate returned
`archivist-authenticated-product-vertical-qualified` with public-safe evidence
SHA-256 `9ec9e373...`: eight synchronized authenticated owners, 10/10 exact
terminals, nine staged requests, one queued cancellation, eight exact source
admissions/staged generations, zero active generations, exact replay, two
PostgreSQL restart/read-backs, and complete teardown. Public exact-head checks
are green: server **1,548 = 1,501 passed + 47 declared skips**, governed fixed
set **173 = 169 + 4**, desktop **59 files / 374 tests**, Playwright **42/42**,
production TypeScript/Vite, desktop Rust **1,247 = 1,236 passed + 11 expected
ignored** plus all integration groups, strict Clippy, orchestrator Rust,
required native WDIO **15/15**, and focused gate **8/8**. The exact successor
closes the native cancellation/quit ownership gap: the client retains its
connection lease until an exact terminal request/source identity is reconciled.
Hosted head `69215c43437c75d7ca2498154a80c4ae5bb749ed` passed all 12
required checks, and PR #177 merged the vertical as
`e397af8b29737fa21197c7058c659eab2ad0a00b`.
The private gate qualifies the authenticated server/database/broker boundary;
native/renderer behavior is exact-head public-test evidence rather than a
private end-to-end client run. See the
[Archivist product record](evidence/archivist-product-vertical/VERIFICATION.md).
...
The current batch-invariant successor was freshly exercised rather than
inheriting that historical receipt. At lock-only head `8fee7a5c...`, the
unchanged executable inputs returned
`scribe-transcript-correction-qualified` with public-safe evidence SHA-256
`7ef51c6d...`: all 24 cases, eight owners, 16 distinct real-ASR English/Spanish
...
privately qualified the merged Student product vertical: authenticated
...
checks. This qualifies the authenticated server boundary, not a live native/
renderer round trip or enterprise identity-provider exchange. Hosted head
`53ce570bf2aedafa1d2d2aebdbcc19349e904ce4` passed all 12 required checks,
and PR #178 merged the vertical as
`6546970ba3613fe55458b54c334a687cb7ff823e`. See the
[Student product record](evidence/student-product-vertical/VERIFICATION.md).

Exact executable successor `6aa33e4d975602c2b6916f1c85a9a782f93d7234`
privately qualified the merged Curator product vertical: authenticated asynchronous
proposal jobs, native-owned bearer/lifecycle/result validation, and one
explicit reviewed-proposal action after a completed Student question. Its
owner-private ARM64 gate matched 10/10 exact terminals across 8 owners (4
proposed, 4 rejected, 1 failed, 1 cancelled), preserved source truth, observed
complex c8 with a ninth owner queued, completed two PostgreSQL restart/read-
backs, and tore down every owned resource. Public-safe evidence SHA-256 is
`328f6640ff945db4ecf224d58861e0c893dcd2f05d2b8ecc70364fb1b1368036`.
A successful result remains noncanonical and review-required; it cannot
activate knowledge or mutate source truth. Public checks are green: portable
server **1,584 = 1,537 passed + 47 declared skips**, governed fixed **173 = 169
+ 4**, focused server/Curator **78/78**, desktop **61 files / 383 tests**,
production TypeScript/Vite, Rust formatting/strict Clippy/tests, and product-
gate contract **7/7**. Hosted head
`b983adb707dffe693de3a9fef8611c3c52fc371a` passed all 12 required checks,
and PR #179 merged the vertical as
`70303872261667da17becfd06f985bc8cda960bc`. The private gate did not execute
a native/renderer round trip or live enterprise identity-provider exchange. See the
...
privately qualified the merged Analyst product vertical: authenticated
asynchronous cited-answer jobs, native-owned bearer/lifecycle/citation
validation, and one explicit cited-answer composer beside permission-safe
Librarian evidence. Its owner-private ARM64 gate matched 10/10 exact terminals
across 8 owners (4 complete, 5 unavailable, 0 failed, 1 cancelled), produced
4/4 exact server-derived answers with server-owned citations, observed strict
owner isolation and hidden/absent equivalence, completed two PostgreSQL restart/
read-backs, contained complex c8 with a ninth owner queued, and tore down every
owned resource. Public-safe evidence SHA-256 is
`f26adfc034cdc99d667f1a99ca54daf92dc9d885456104f17787cdf2cc96fa44`.
Public checks are green: portable server **1,603 = 1,556 passed + 47 declared
skips**, governed fixed **173 = 169 + 4**, focused Analyst **69 = 67 + 2
expected PostgreSQL skips**, desktop **62 files / 387 tests**, production
TypeScript/Vite, whole-server Ruff, Rust formatting/strict Clippy/tests, and
product-gate contract **9/9**. Hosted head
`4c8db7c22f6a8ebadbc73433155f060b5b699fb6` passed all 12 required checks,
and PR #180 merged the product vertical as
`c95fcf1a043f919661b007f014a8dc6729aa02f2`. The private gate qualifies the authenticated
server boundary, not a native/renderer round trip or live enterprise identity-
provider exchange. See the
[Analyst product record](evidence/analyst-product-vertical/VERIFICATION.md).

Exact executable successor `05400fb33a6a2ed63f49db622b8789a330def3e7`
now privately qualifies the Coordinator product candidate: authenticated
asynchronous proposal-bundle jobs, native-owned bearer/lifecycle/citation
validation, and one explicit Coordinator composer. Its owner-private ARM64 gate
matched 10/10 exact terminals across 8 owners (5 complete, 4 unavailable, 0
failed, 1 cancelled), produced 5/5 exact server-derived noncanonical review-
required bundles with server-owned citations, observed strict owner isolation
and hidden/absent equivalence, completed two PostgreSQL restart/read-backs,
contained complex c8 with a ninth owner queued, and tore down every owned
resource. Public-safe evidence SHA-256 is
`394112ade3727f4e40fbec0b5083e23d7889a0318553026227f0c4a0e6c4bd89`.
Public checks are green: portable server **1,622 = 1,575 passed + 47 declared
skips**, governed fixed **173 = 169 + 4**, focused product/core **41/41**,
desktop **63 files / 391 tests**, production TypeScript/Vite, whole-server Ruff,
Rust formatting/strict Clippy/tests, and documentation truth. The candidate
remains unmerged and requires unchanged-head hosted review. The private gate
qualifies the authenticated server/database/broker boundary, not a native/
...
`5c2525b3...`. All eight owners produced source-grounded output with zero
...
sustained mixed-route capacity, production SLOs, internal-role product exposure,
and the two later role workflows remain open.

The batch-invariant admission successor has its own complete evidence chain.
Exact executable candidate `0665c486398d2803ba33ebbb6e6dedddcd844dbd`
passed the sequential supervised-service lifecycle with public-safe evidence
SHA-256 `7cc016f493ab4f14c3b6474e21f59ca33228d66dcc816699282ed31cea3d94d8`,
then returned `required-workload-routes-qualified` with public-safe evidence
SHA-256 `06277bd938314eb86cf5e516e6e6cfda7982652fbe3e1baa4586b6bb88cd5b51`.
Lock-only successor `8fee7a5c4969d482e5248b72013935e3f6f83b3c`
publishes the matching route lock, whose SHA-256 is
`3b16aa134c6af6a6bdc56afe64bbffeead593fa9c11e6db4512be95102c9f19f`.
Qwen and Gemma were admitted sequentially on their unchanged full profiles and
completed exact teardown. The complex profile now owns batch-invariant request
execution with seed `0` and prefix caching disabled.

At the lock-only successor, the unchanged executable inputs freshly requalified
...
complete portable server read-back is 1,501 tests: 1,454 passed and 47 declared
...
not simultaneous residency or sustained-capacity evidence.

Exact route heads `9551532d...` and `98fb89f9...` remain terminal
`deterministic-no-model` evidence with public-safe SHA-256 `4a70cc17...` and
`bbf9e67c...`. They exposed prompt-refusal/tool-selection and answer/citation
field-ownership ambiguities, respectively. Neither receipt is reused, and no
raw output or private measurement is published.

Curator is a merged internal core and product surface. Exact `6aa33e4d...`
privately qualified its authenticated product server boundary, hosted head
`b983adb7...` passed all 12 checks, and PR #179 merged the HTTP/native/renderer
vertical as `70303872...`. At lock-only successor `8fee7a5c...`, the unchanged
internal-core inputs returned
...
Its exact cases, owner, publication, database, warm-provider, broker, and
teardown boundaries passed against the new profile/runtime. Curator
accepts only an explicit proposal or reviewed Student answer, may publish only a
...
#168 merged the internal core as `284ab96b...` on 2026-08-13. Its prior exact
`7cd24deb...` qualification remains merge evidence; the successor receipt does
not relabel it.

Librarian is a merged internal core. Exact head `56b7f5d0...` supplied its
qualification, hosted head `7505247e...` merged through PR #169 as
`d7a7e003...`, and lock-only successor `8fee7a5c...` freshly returned
...
three unavailable, one stale failure, and two cancellations. The actual
synchronized wave admitted all eight normal owners through the broker; the
Server-IO probe admitted one, queued the second, completed cancellation and
acknowledgement, retained broker identity, and acquired no model lease. Hidden
results/links, revocation, stale generation, deadline, cancellation, exact
audits, two PostgreSQL restart read-backs, zero proposals, retained generation/
admission identities, and all six teardown assertions held. The p95 stayed
within the frozen 16-second bound. Exact predecessor `ecdcb8ee...` and evidence
SHA-256 `83f618c6...` are terminal/inadmissible because adversarial review found
only seven broker submissions behind eight owner labels; that receipt is not
reused. An earlier publication attempt minted no receipt after its destination
parent failed the owner-private check. No private path, content, identifier,
envelope hash, or individual latency is published. Librarian's separate product
surface later merged through PR #174. See the
...
15 server-owned citations. Empty/unavailable, stale, invalid, cancellation,
deadline, replay, and cross-owner paths failed closed; exact database read-back,
provider/broker identity, capacity-probe containment, and teardown held. The
three repeats prove same-warm-process repeatability, not cross-start/global
determinism. Exact `63c3d9fd...` produced conflicting official/diagnostic
...
`52c45d22...`. Exact product successor `78b2c638...` added and privately
qualified the authenticated HTTP server boundary plus public-test-green native/
renderer composition. Hosted head `4c8db7c2...` passed all 12 checks and PR #180
merged it as `c95fcf1a...`.

Coordinator is a privately qualified merged internal core at exact executable
...
Hosted head `53ee0152...` passed all 12 required checks, and PR #171 merged the
core as `67d836da...`. Exact product successor `05400fb3...` now adds and
privately qualifies the authenticated HTTP server/database/broker boundary plus
public-test-green native/renderer composition; it remains unmerged pending
hosted review.

Auditor privately qualified at exact executable head
`08b06f6db45e233d92035788dabdee7157a83d2f`. Its
`auditor-source-cited-review-findings-qualified` public-safe evidence SHA-256 is
`2c1dbc0552233c377f0eff35bcfda3a5d761dbb548d06e638426cd232bddc376`.
Three synchronized eight-owner idle-only service waves matched all 24 normal
calls, and all 29 unique terminal outcomes matched: 12 complete reports, 13
unavailable, one failed invalid-output control, and three cancellations. The
server derived 15 potential-contradiction findings and 30 citations from
current owner-visible source evidence; every report was noncanonical and
review-required. Active and queued non-idle work each blocked Auditor
admission, which resumed only after non-idle work became terminal. One Auditor
lease per invocation, c8/ninth-queued containment, two PostgreSQL restart/read-
backs, exact content-free audits, zero proposal writes, and complete teardown
passed. Auditor writes no proposal, source, active knowledge, task, or action
and performs no autonomous mutation. See the
[Auditor evidence](evidence/auditor-source-cited-review-findings/VERIFICATION.md).
Hosted head `937a4129e13d70e68c6d8151cf9643512b6b6e16` passed all 12
...
Curator, and Analyst are the six merged product surfaces. Librarian merged
through PR #174, Archivist through PR #177, Student through PR #178, Curator
through PR #179, and Analyst through PR #180, with each private gate retaining
its exact server-boundary attribution. Coordinator is a privately qualified,
unmerged product candidate at exact `05400fb3...`. Auditor product exposure
remains pending.
...
`0.70` profiles do not fit together on one Spark; a second owned GPU node and
IT-controlled private route remain required before warm two-route promotion.
...
before completing the JSON response. Exact successor
`21559371db2a869e2c8b7ae3cd589f80c189d0cd` raised only that workload allowance
to 512 and also returned terminal `deterministic-no-scribe`, with public-safe
evidence SHA-256
`a103144c66940ff55d8390c227bc73e6379cbfa6f73a199a9818839adaf48e2b`.
All 24 requests completed across eight owners and 16 unique real-audio items;
the warm model, owner-fair broker, queue-inclusive latency, protected-fact and
no-regression checks, database teardown, and runtime teardown held. Complete
edited JSON exposed the next exact failure: model-authored character offsets did
not consistently bind the quoted source. The protected response-schema-v2
successor removes offset authority from the model. It accepts only an exact
...
bounded private smoke proved that complete responses now pass schema, request,
segment, exact-quote, derived-span, and bounded-edit validation. The safety case
passed once but was not stable across repeated cold diagnostic runs, and a real
case selected from the prior invalid-output set attempted an unauthorized name
change that was neither approved terminology nor present in the frozen human
...
**Most recent merged checkpoint candidate:** exact aggregate head
`22c3f3698a6b5c5ff592e74f3a0f0e144778c9c5` passed the governed-knowledge
maintainability gate after exact private route qualification and semantic
admission. Final hosted head `84c22ec9935af824ca1b47d046e18003ec2c7883`
passed frontend, Rust, server, mock OIDC, required native WDIO, and every
CodeQL lane before PR #153 merged it as
...
head `36350d449735a4daea6546e16759f28f6f15631a` returned
`required-workload-routes-qualified` for both locked vLLM routes with
public-safe evidence SHA-256
`ca5a3f712ff737b92cc0d17979e5cd5b00e3034c880729e290a6f6ba255ca951`. Exact
candidate `a4f34678ea9980379b18266d40d3347b818ac57e` then passed the one complete
...
post-Phase-9 maintainability checkpoint is merged and closed. A prior private run at
exact head `96897d2f77f16457a9da2b87af8a9bf4c9ad2b99` completed both owned
lifecycles and teardown but returned `deterministic-no-model`; public-safe
evidence SHA-256
`929dd2a329387e0647db49699b0653862668f8f6b4588a4bf3ee9818ba656b75`
is retained only as terminal rejected evidence. Gemma remained eligible. Qwen
passed every frozen semantic contract plus its common, warm, and C8 route
bounds, but the single aggregate fixture latency policy treated citation-rich
structured proposals like common governed reads and caused rejection.
Reviewed executable head
`a76ed9b095ebb797064a12e9ebd90d2dd9d87bef` retains the complete synthetic
proposal contract, two evaluation-only final structural-decoding attempts, and
the candidate-specific Qwen 26.07+XGrammar 0.2.1 / Gemma 26.06 runtimes. Its
schema-4 acceptance keeps Qwen's route maximum at 256 tokens, applies 160 only
to the three proposal fixtures, and separates three-second common from
ten-second proposal qualification bounds; Gemma remains at 512. Every
correctness threshold, warm/C8 bound, model, route, runtime, and no-tool-retry
rule remains unchanged. Candidate-lock SHA-256 is
...
Focused public checks passed. The fresh qualification at that exact clean head
...
six teardown predicates with zero owned residue. Final hosted head
`84c22ec9935af824ca1b47d046e18003ec2c7883` passed frontend, Rust, server,
mock OIDC, required native WDIO, and every CodeQL lane; PR #153 merged it as
`ca151b1b45be3b98e4c56c6ea2b89446eeaa8814`. These frozen evaluation bounds
are not a production p95/p99 SLO,
generic TPS claim, production agent service, simultaneous-residency result, or
sustained mixed-route capacity proof.

Phase 10 Slices 10.1 and 10.2 are merged. Slice 10.2 adds immutable Qwen rapid
and Gemma complex service
...
fair admission; requests do not launch or swap models. The merged historical
admission-core head `7bd93dc6...` implemented all eight role bindings, one-
...
and cancellation acknowledgement, provider-generation disruption, strict
...
| Phase 10 | Eight bounded role cores plus six product surfaces merged; Coordinator product successor privately qualified and awaiting hosted merge | PRs #164/#165/#166/#168/#169/#170/#171/#172 merged all eight bounded internal cores. PRs #174/#177/#178/#179/#180 merged Librarian, Archivist, Student, Curator, and Analyst product verticals after their exact private server-boundary qualifications; Scribe was already merged through PR #164. Exact `05400fb3...` privately qualified Coordinator's authenticated product server/database/broker boundary with 10/10 exact terminals, 5 exact server-derived noncanonical review-required bundles, strict owner isolation, two PostgreSQL restart/read-backs, complex-c8/ninth-owner-queued containment, and public-safe evidence SHA-256 `394112ad...`. Coordinator hosted review/merge, the Auditor product surface, simultaneous full-profile residency, sustained capacity/SLOs, production operations, and explicit IT/security handoffs remain open. |
...
Replacement head `06a66435880f19812eecafaec65e6cb9f201b646`
passed the canonical build and exact-image product lifecycle: the real Tiron
result and speaker revision opened from History, active cancellation published
nothing, and independent teardown proved zero retained owners. Its complete
matrix then stopped at command cell 11 before native tests because Windows
PowerShell 5.1 could not apply the private DACL to a 273-character zero-byte
command-log path. No candidate receipt was published. The runner now routes ACL
I/O through Windows extended-length paths while preserving canonical receipt
paths; an open-descriptor regression covers the exact production seam.

The next clean head `b9ef6ff4d7c727aabb5f40a9c781ae6ebd06ca32`
passed its canonical build, exact Tiron image preparation, server preflight,
and admission. Its product evidence is intentionally invalid: adversarial
review found that the private operator controller used check-then-create and
reopened its cleanup logs instead of retaining exclusively created handles.
The run was stopped before inference; remote cleanup passed and direct
inspection found zero local or remote owners, but only a gate context exists—no
vertical, cancellation, or independent teardown receipt. That admission is
historical evidence and cannot qualify a candidate. The corrected controller
uses one `CreateNew` stream per log from ACL protection through final disposal,
and the runbook now makes that invariant explicit.

Candidate head `6d52be5bcf2f7a0f2f9607f80f3d8c1ec6121e5a`
passed its exact-image product lifecycle and all five private receipts. Its
candidate matrix then passed frontend and Windows prerequisite checks, native
formatting and Clippy, and all 1,198 Rust tests before the strict product Job
reported one retained descendant after the cold MSVC link. Focused cold-link
reproduction identified the signed Visual Studio `vctip.exe` helper and no Rust
test leak; extending the drain, disabling optional diagnostics, and disabling
test debug information did not stop that resident helper. The admission remains
failed historical evidence. The meeting checkpoint now follows the existing
Phase 7 boundary: its local candidate owns frontend/server/private lifecycle
evidence and the Windows prerequisite, while native formatting, Clippy,
compilation, tests, connector/dependency checks, and WDIO build evidence execute
in the clean exact-head hosted Rust and native-WDIO jobs. Runtime connector and
application trees remain strictly contained, with no compiler exception.
...
passed CodeQL, but its first hosted frontend audit failed on newly visible
high-severity `GHSA-mwp4-54f8-5fhr`; the remaining CI and NSIS work was canceled
because that head could not merge. The patched `ip-address` 10.3.1 override is
development-only, but its lockfile change invalidates the documentation-only
lineage. It received a fresh candidate gate rather than an exception.

Patched candidate `393710999b53a4bd1b00639e30c0fec88b152530` passed the
canonical native build, exact-image preparation and server preflight, real
desktop-to-GB10 History and active-cancellation lifecycle with independent
teardown, its one complete 18-child checkpoint matrix, and independent
candidate-receipt validation. The same exact head passed every required hosted
CI and CodeQL job, and the reviewed successor merged through PR #143 as
...
`a92f338546a2f8bbaded96b04f8987f0ac475c88`: all 18 candidate-safety children
...
loopback-proxy process-group identity; verified blocked container egress; ran
the plan-owned duration/load/cancellation/capacity/resource cells; and
published only after complete child evidence plus clean launcher, proxy,
container, network, and host teardown. Representative quality, provider
promotion, and later production capacity remain open.
...
- Purpose-control is storage plus a decision service with no caller, not
  executable authorization. `IdentityAuthorizationService` implements grant,
  revoke, and access-control decisions with redacted audit records, and its unit
  tests exercise them, but no HTTP route, CLI, or operator entry point invokes
  it: `RequestAuthorizationRuntime.purpose_authorization` is an optional field
  that nothing reads, and `Yap.IdentityAdministrator` appears only in that
  module and as a settings default. In practice a grant can be created or
  `access_disabled` set only by editing `identity.sqlite` by hand. Enrollment,
  matching, and adaptation do not exist to require grants. Treat this as a Phase
  8 gate: the primitive ADR 0016 asks for is built and unwired, not enforcing.
  No voice profile or embedding implementation is implied.
...
completed Phase 6 plan. ADR 0027 now executes a candidate meeting-result path;
the model remains evidence while the authenticated job ledger and native
publication boundaries retain ownership.
...
- The merged Phase 6 implementation includes focused slices: a verified
  ...
  `a92f338546a2f8bbaded96b04f8987f0ac475c88` passed current-host
  ...
  `a92f338546a2f8bbaded96b04f8987f0ac475c88`. They remain
...
vLLM/Nemotron NeMo suite and exact teardown. The connected channel proved one
immutable job across a stopped and restored SSH forward, durable
preprocessing, the five-window AmberNet preflight, server-authoritative result
publication, verified History review, and exact teardown. The complete command
matrix passed frontend accessibility/workflows, production build,
release/provenance and dependency contracts, Rust format/Clippy/tests/audit/
Windows boundary, server-connector integration, required native WDIO, and the
portable Python 3.12 server suite.

Private audio, transcripts, raw metrics, process ledgers, host paths, and logs
remain outside Git and hosted artifacts. Historical candidate
`e4a62f4b8914e9233cd5229fa8f134f0c59fdbbb` was invalidated after final
adversarial review found that concurrent model/language mutation could cancel
...
18-child GB10, connected interruption/recovery/History, and teardown channels.
Its public-safe GB10 aggregate file SHA-256 was
`b8daa673febc3fb7777ea099c84878bb929ea2ce49d2f3a70c17b0baf594bc78`.
Hosted Windows execution then exposed a test-fixture portability defect:
release-contract fixtures inherited GitHub Runner's redirected temporary
parent and were rejected before reaching their intended containment cases.
The test-only canonical-temporary-root correction invalidated `0ed2037...` as
merge authority. Replacement exact candidate
`e4a62f4b8914e9233cd5229fa8f134f0c59fdbbb` subsequently rebuilt the checked
runtime preparations and reran every admitted channel and all 30 children after
the complete executable and controller correction sequence; no prior receipt
or child was relabeled or reused.
...
2. Preserve the merged Librarian, Archivist, Student, Curator, and Analyst product
   ...
   unchanged-head hosted review/merge for the privately qualified Coordinator
   product successor, then deliver the Auditor product surface.
...
3. Provision a second full-strength owned GPU node and IT-controlled private
   ...
   request; run sustained mixed-owner/mixed-route capacity evidence only on the
   admitted topology.
