...
foreach ($RequiredPort in @($HttpPort, $LivePort)) {
    if (Test-LoopbackListener -Port $RequiredPort) {
        throw "The authenticated connector gate requires port $RequiredPort to be unowned."
    }
}
...
try {
    ...
    for ($Attempt = 0; $Attempt -lt 30; $Attempt++) {
        if (
            ...
            (Test-LoopbackListener -Port $HttpPort) -and
            (Test-LoopbackListener -Port $LivePort)
        ) {
            $Ready = $true
            break
        }
    ...
    $env:YAP_TEST_AUTH_SERVER_URL = "http://127.0.0.1:$HttpPort"
    $env:YAP_TEST_AUTH_SERVER_LIVE_ORIGIN = "http://127.0.0.1:$LivePort"
    ...
    Remove-Item Env:YAP_TEST_AUTH_SERVER_LIVE_ORIGIN -ErrorAction SilentlyContinue
    Stop-Process -Id $Server.Id -Force -ErrorAction SilentlyContinue
    $Server.WaitForExit(5000) | Out-Null
    foreach ($RequiredPort in @($HttpPort, $LivePort)) {
        $Deadline = [DateTime]::UtcNow.AddSeconds(5)
        while (Test-LoopbackListener -Port $RequiredPort) {
            if ([DateTime]::UtcNow -ge $Deadline) {
                throw "Port $RequiredPort remained owned after authenticated connector teardown."
            }
