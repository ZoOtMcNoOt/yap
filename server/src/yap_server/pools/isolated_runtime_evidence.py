...
def _read_regular_file(path: Path, *, limit_bytes: int) -> bytes:
    ...
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise RuntimeError(f"runtime evidence input is not a regular file: {path.name}")
    if metadata.st_size > limit_bytes:
        raise RuntimeError(f"runtime evidence input exceeds its bound: {path.name}")
    ...
    *,
    ...
    for name in _UNCHANGED_SNAPSHOTS:
        if before[name] != after[name]:
            raise RuntimeError(f"runtime gate changed observed host state: {name}")
    ...
    for name in _EMPTY_RUNTIME_SNAPSHOTS:
        ...
        if before_value.strip():
            raise RuntimeError(f"runtime gate started with residual state: {name}")
        if after_value.strip():
            raise RuntimeError(f"runtime gate left residual state: {name}")
    ...
    try:
        ...
        raise RuntimeError("inference evidence is invalid JSON") from error
    if not isinstance(evidence, dict) or evidence.get("schemaVersion") != 1:
        raise RuntimeError("inference evidence has an invalid schema")
    if evidence.get("resultSha256") != _digest(result_bytes):
        raise RuntimeError("result digest does not match its inference evidence")
    boundary = evidence.get("boundary")
    if not isinstance(boundary, dict) or (
        boundary.get("network") != "none" or boundary.get("workerCount") != 1
    ):
        raise RuntimeError("inference evidence has an invalid worker boundary")
    ...
    if (
        ...
        raise RuntimeError("result and evidence need one dedicated publication directory")
    ...
    try:
        ...
        raise RuntimeError(
            "checked-head evidence directory already exists"
        ) from error
...
def _write_bytes_atomic(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError("runtime evidence output already exists") from error
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Finalize isolated-runtime evidence after host-boundary teardown"
    )
