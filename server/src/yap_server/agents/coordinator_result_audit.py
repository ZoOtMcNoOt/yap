...
_AGENT_ROLE = "coordinator"
_PURPOSE = "conversation-coordinate"
_ROUTE = "complex-orchestration"
_SCHEDULING_CLASS = "background-llm"
COORDINATOR_RESULT_AUDIT_CONNECTION_BUDGET_SECONDS = 3.1
_OUTCOMES = {
    "complete": "succeeded",
...
class CoordinatorRuntimeAuditIdentity:
    candidate_id: str
    model: str
    model_revision: str
    runtime_id: str
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("coordinator runtime audit identity is invalid")


@dataclass(frozen=True, slots=True)
class CoordinatorStoredResult:
    request_id: str
    request_sha256: str
...
def install_coordinator_result_audit_schema(connection: Connection[object]) -> None:
    """Install the immutable, content-free Coordinator terminal-result ledger."""

    with connection.transaction():
        connection.execute(
            """CREATE TABLE IF NOT EXISTS yap_coordinator_result_audit (
                ...
                agent_role varchar(16) NOT NULL CHECK (agent_role = 'coordinator'),
                purpose varchar(32) NOT NULL
                    CHECK (purpose = 'conversation-coordinate'),
                route varchar(32) NOT NULL CHECK (route = 'complex-orchestration'),
                scheduling_class varchar(16) NOT NULL
                    CHECK (scheduling_class = 'background-llm'),
                ...
                CHECK (
                    (bundle_sha256 IS NULL AND citation_sha256 IS NULL)
                    OR
                    (bundle_sha256 IS NOT NULL AND citation_sha256 IS NOT NULL)
                ),
                ...
                CHECK (
                    ...
                    ) AND bundle_sha256 IS NULL AND citation_sha256 IS NULL
                        AND result_count = 0)
                    OR
                    (outcome = 'cancelled'
...
class PostgresCoordinatorResultAuditor:
    """Persist one exact Coordinator outcome without objective or bundle bytes."""

    def __init__(
        self,
        connection_factory: PrivatePostgresConnectionFactory,
        runtime_identity: CoordinatorRuntimeAuditIdentity,
    ) -> None:
        if not isinstance(runtime_identity, CoordinatorRuntimeAuditIdentity):
            raise TypeError("coordinator runtime audit identity type is invalid")
    ...
    def read(
        ...
        *,
        ...
        request_id: str,
    ) -> CoordinatorStoredResult | None:
        if (
            not isinstance(principal, AuthenticatedPrincipal)
            or not isinstance(request_id, str)
            or _REQUEST_ID.fullmatch(request_id) is None
        ):
            raise ValueError("coordinator stored result lookup is invalid")
        ...
        if row is None:
            return None
        status = _STORED_STATUSES.get(str(row[16]))
        if status is None:
            raise ValueError("coordinator stored result outcome is invalid")
        return CoordinatorStoredResult(
            ...
            bundle_sha256=(str(row[4]) if row[4] is not None else None),
            citation_sha256=(str(row[5]) if row[5] is not None else None),
            generation_sha256=(str(row[6]) if row[6] is not None else None),
            permission_hash=(str(row[7]) if row[7] is not None else None),
            authorization_hash=(str(row[8]) if row[8] is not None else None),
            provider_generation=(int(row[9]) if row[9] is not None else None),
            candidate_id=str(row[10]),
            model=str(row[11]),
            model_revision=str(row[12]),
            runtime_id=str(row[13]),
            profile_sha256=str(row[14]),
            candidate_lock_sha256=str(row[15]),
            status=status,
            reason=(str(row[17]) if row[17] is not None else None),
            result_count=int(row[18]),
            duration_milliseconds=int(row[19]),
        )
    ...
    def record(
        ...
        *,
        ...
        if (
            ...
            raise ValueError("coordinator result audit is invalid")
        if (
            deadline - time.monotonic()
            <= COORDINATOR_RESULT_AUDIT_CONNECTION_BUDGET_SECONDS
        ):
            raise KnowledgeToolTimedOut(
                "coordinator result audit lacks a bounded connection window"
            )
        ...
        with self._connection_factory() as connection:
            ...
            if remaining_seconds <= 0:
                raise KnowledgeToolTimedOut(
                    "coordinator result audit exceeded its deadline"
                )
            ...
            forwarder = threading.Thread(
                ...
                name="coordinator-result-audit-cancellation",
                daemon=False,
            )
            ...
            deadline_timer.name = "coordinator-result-audit-deadline"
            ...
            def persist() -> None:
                with connection.transaction():
                    ...
                    if status == "complete":
                        ...
                        current = read_coordinator_evidence_in_transaction(
                            connection,
                            request,
                            principal=principal,
                        )
                    ...
                    if cancellation.is_set():
                        raise KnowledgeToolCancelled(
                            "coordinator result audit was cancelled before commit"
                        )
                    if deadline <= time.monotonic():
                        raise KnowledgeToolTimedOut(
                            "coordinator result audit exceeded its deadline before commit"
                        )
            ...
            try:
                try:
                    ...
                    if operation_cancellation.is_set():
                        raise KnowledgeToolCancelled(
                            "coordinator result audit was cancelled"
                        ) from error
                    raise
            finally:
    ...
    def _values(
        ...
        *,
        ...
        if (
            not isinstance(principal, AuthenticatedPrincipal)
            or not isinstance(request, CoordinatorRequest)
            or not isinstance(request_id, str)
            or _REQUEST_ID.fullmatch(request_id) is None
            or (
            ...
            or (
                ...
                and not isinstance(evidence, CoordinatorEvidencePack)
            )
            or (
                bundle is not None and not isinstance(bundle, CoordinatorProposalBundle)
            )
            or (evidence is None and bundle is not None)
            or (evidence is not None and provider_generation is None)
            or (status == "complete" and (evidence is None or bundle is None))
            or (status == "complete" and provider_generation is None)
            or (status != "complete" and bundle is not None)
            or (
                reason
                in {
                    "invalid-output",
        ...
        work_sha256 = (
            coordinator_work_sha256(request, evidence) if evidence is not None else None
        )
    ...
    def _recover_exact(
        ...
        *,
        ...
        with self._connection_factory() as connection:
            ...
            deadline_timer.name = "coordinator-result-audit-recovery-deadline"
            deadline_timer.daemon = False
            ...
            def recover() -> tuple[object, ...] | None:
                ...
                if deadline <= time.monotonic():
                    raise KnowledgeToolTimedOut(
                        "coordinator result audit recovery exceeded its deadline"
                    )
                connection.execute(
        ...
        if tuple(stored) != values:
            raise ValueError("coordinator result audit identity conflicts")
        return True
...
def _insert_or_verify(
    ...
    inserted = connection.execute(
        """INSERT INTO yap_coordinator_result_audit (
            tenant_id, subject_id, request_id, request_sha256, work_sha256,
            evidence_sha256, bundle_sha256, citation_sha256, generation_sha256,
            permission_hash, authorization_hash, agent_role, purpose, route,
...
def _read_stored(
    ...
    return connection.execute(
        """SELECT tenant_id, subject_id, request_id, request_sha256,
                  work_sha256, evidence_sha256, bundle_sha256,
