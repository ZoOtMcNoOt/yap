...
class CoordinatorProductRuntimeTests(unittest.TestCase):
    def test_absent_core_keeps_product_surface_disabled(self) -> None:
        with mock.patch(
            "yap_server.agents.coordinator_product_runtime.build_coordinator_runtime",
            return_value=None,
        ) as build_core:
    ...
    def test_product_runtime_wraps_and_closes_exact_core_service(self) -> None:
        ...
        with (
            ...
            mock.patch(
                "yap_server.agents.coordinator_product_runtime.CoordinatorBundleService",
                return_value=product_service,
            ) as product_type,
        ):
            runtime = build_coordinator_product_runtime(
                {"YAP_COORDINATOR_RUNTIME": "warm_gemma"},
                authenticated_team_mode=True,
            )
        ...
        product_type.assert_called_once_with(coordinator=core_service)
        runtime.close()
