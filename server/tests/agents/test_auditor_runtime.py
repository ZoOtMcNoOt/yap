...
def _environment(socket_path: Path, dsn_path: Path) -> dict[str, str]:
    return {
        ...
        AUDITOR_ADMISSION_SOCKET: str(socket_path),
        AUDITOR_PROFILE: str(PROFILE),
        AUDITOR_CANDIDATE_LOCK: str(CANDIDATE_LOCK),
...
class AuditorRuntimeTests(unittest.TestCase):
    ...
    def test_runtime_is_disabled_only_without_auditor_paths(self) -> None:
        self.assertIsNone(build_auditor_runtime({}, authenticated_team_mode=True))
        self.assertIsNone(
            build_auditor_runtime(
                {AUDITOR_RUNTIME: "disabled"},
        ...
        environment[AUDITOR_RUNTIME] = "disabled"
        with self.assertRaisesRegex(ValueError, "disabled auditor"):
            build_auditor_runtime(environment, authenticated_team_mode=True)
    ...
    def test_runtime_shares_admission_and_database_factory_without_lifecycle(
        ...
        with (
            patch(
                "yap_server.agents.auditor_runtime.UnixAgentAdmissionTransport",
                return_value=admission_transport,
            ) as admission_transport_constructor,
            patch(
                "yap_server.agents.auditor_runtime.AgentAdmissionClient",
                return_value=admission,
            ) as admission_constructor,
            patch(
                "yap_server.agents.auditor_runtime.BoundedVllmJsonClient",
                return_value=transport,
            ) as transport_constructor,
            patch(
                "yap_server.agents.auditor_runtime.private_postgres_connection_factory",
                return_value=connection_factory,
            ) as connection_factory_constructor,
            patch(
                "yap_server.agents.auditor_runtime.PostgresAuditorEvidenceReader",
                return_value=evidence_reader,
            ) as reader_constructor,
            patch(
                "yap_server.agents.auditor_runtime.AuditorEvidenceModel",
                return_value=model,
            ) as model_constructor,
            patch(
                "yap_server.agents.auditor_runtime.PostgresAuditorResultAuditor",
                return_value=result_auditor,
            ) as result_auditor_constructor,
            patch(
                "yap_server.agents.auditor_runtime.AuditorService",
    ...
    def test_mode_paths_and_profile_bytes_fail_closed(self) -> None:
        ...
        environment[AUDITOR_RUNTIME] = "warm_qwen"
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            environment = self._environment(socket_path)
            environment[AUDITOR_PROFILE] = str(changed_profile)
            ...
            environment = self._environment(socket_path)
            environment[AUDITOR_CANDIDATE_LOCK] = str(changed_lock)
