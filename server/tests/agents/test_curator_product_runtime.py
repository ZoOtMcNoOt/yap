...
from yap_server.agents.curator_product_runtime import build_curator_product_runtime


class CuratorProductRuntimeTests(unittest.TestCase):
    def test_absent_core_keeps_product_surface_disabled(self) -> None:
        with mock.patch(
            "yap_server.agents.curator_product_runtime.build_curator_runtime",
            return_value=None,
        ) as build_core:
            runtime = build_curator_product_runtime({}, authenticated_team_mode=True)
    ...
    def test_product_runtime_wraps_and_closes_exact_core_service(self) -> None:
        ...
        with (
            ...
            mock.patch(
                "yap_server.agents.curator_product_runtime.CuratorProposalService",
                return_value=product_service,
            ) as product_type,
        ):
            runtime = build_curator_product_runtime(
                {"YAP_CURATOR_RUNTIME": "warm_gemma"},
        ...
        product_type.assert_called_once_with(curator=core_service)
        runtime.close()
