...
from yap_server.evaluation import analyst_product_qualification_gate as gate
from yap_server.evaluation.analyst_qualification import AnalystExpectedView
...
def _answer() -> AnalystAnswer:
    item = LibrarianEvidenceItem(
        concept_id="analyst-product-test",
    ...
    return AnalystAnswer(
        answer=item.text,
        citations=(item,),
        answer_sha256=hashlib.sha256(item.text.encode()).hexdigest(),
        citation_sha256=analyst_citation_sha256((item,)),
        evidence_sha256="b" * 64,
    )


def _request(index: int = 0) -> AnalystRequest:
    return AnalystRequest(
        question=f"What is product fact {index}?",
        maximum_results=3,
        expected_generation_sha256="c" * 64,
    )


class AnalystProductQualificationGateTests(unittest.TestCase):
    def test_acceptance_and_candidate_inputs_cover_the_product_boundary(self) -> None:
        acceptance = gate.load_analyst_product_acceptance(
            REPOSITORY_ROOT / "server/analyst-product-acceptance.json"
        )
        ...
        required = {
            "server/analyst-product-acceptance.json",
            "server/src/yap_server/agents/analyst_answer_service.py",
            "server/src/yap_server/agents/analyst_product_runtime.py",
            "server/src/yap_server/api/analyst_answer_requests.py",
            "server/src/yap_server/evaluation/analyst_product_qualification_gate.py",
            "server/tests/evaluation/test_analyst_product_qualification_gate.py",
            "desktop/src/analyst.ts",
            "desktop/src/components/analyst/analyst-answer-composer.tsx",
            "desktop/src/components/analyst/analyst-answer-result.tsx",
            "desktop/src/components/analyst/use-analyst-answer.ts",
            "desktop/src-tauri/src/analyst_answer.rs",
            "desktop/src-tauri/src/server_connector/analyst.rs",
            "desktop/tests/unit/analyst-product.test.tsx",
    ...
    def test_product_view_parser_is_strict_and_hash_bound(self) -> None:
        answer = _answer()
        request_id = "analyst-answer-" + "1" * 32
        complete = {
            "schemaVersion": 1,
            "requestId": request_id,
            "status": "complete",
            "citedAnswer": answer.to_wire(),
        }
        ...
        forged = answer.to_wire()
        forged["answerSha256"] = "d" * 64
        with self.assertRaisesRegex(ValueError, "analyst answer"):
            gate._parse_product_view(dict(complete, citedAnswer=forged))

    def test_active_and_unavailable_views_carry_no_answer(self) -> None:
        request_id = "analyst-answer-" + "2" * 32
        ...
        self.assertIsNone(active.answer)
        self.assertIsNone(unavailable.answer)
        with self.assertRaisesRegex(ValueError, "fields"):
    ...
    def test_http_authentication_covers_post_get_and_delete(self) -> None:
        def response(_base_url, path, *, method, token, body=None):
            ...
            if path == "/v1/health":
                return 200, {
                    "auth": "required",
                    "capabilities": {"analystAnswers": True},
                }
    ...
    def test_foreign_owner_cannot_read_or_cancel(self) -> None:
        with mock.patch.object(
            ...
            "_http_json",
            side_effect=(
                (404, {"code": "ANALYST_ANSWER_NOT_FOUND"}),
                (404, {"code": "ANALYST_ANSWER_NOT_FOUND"}),
            ),
        ) as call:
    ...
    def test_product_evidence_counts_and_hidden_shape_fail_closed(self) -> None:
        ...
        answer = _answer()
        observations: list[gate.AnalystProductObservation] = []
        labels = (
            "exact-single-answer",
            "numeric-unit-answer",
            "ordered-multi-answer",
            "instruction-as-data-answer",
            "missing-date-unavailable",
            "unassigned-owner-unavailable",
            "unstated-rationale-unavailable",
            "absent-time-unavailable",
        )
        for index, label in enumerate(labels):
            expected = (
                AnalystExpectedView("complete", None, answer)
                if index < 4
                else AnalystExpectedView(
                    "evidence-unavailable",
                    "empty-result"
                    if label == "absent-time-unavailable"
                    else "model-evidence-unavailable",
                    None,
                )
            )
            view = gate.AnalystProductView(
                "analyst-answer-" + f"{index:032x}",
                expected.status,
                expected.answer,
                expected.reason,
            )
            observations.append(
                gate.AnalystProductObservation(
                    label,
                    f"owner-{index}",
                    _request(index),
                    expected,
        ...
        empty = AnalystExpectedView("evidence-unavailable", "empty-result", None)
        hidden_view = gate.AnalystProductView(
            "analyst-answer-" + "a" * 32,
            empty.status,
            None,
            empty.reason,
        ...
        cancelled = AnalystExpectedView("cancelled", "client-cancelled", None)
        cancelled_view = gate.AnalystProductView(
            "analyst-answer-" + "b" * 32,
            cancelled.status,
            None,
            cancelled.reason,
    ...
    def test_gate_owns_two_restarts_http_runtimes_and_private_receipt(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as value:
            ...
            patches = {
                ...
                "load_analyst_product_acceptance": {"return_value": acceptance},
                "load_analyst_qualification_acceptance": {
                    "return_value": semantic_acceptance
                },
                "load_analyst_qualification_corpus": {"return_value": corpus},
                "load_analyst_service_profile": {"return_value": profile},
                ...
                "_initialize_analyst_knowledge": {"return_value": initialized},
                ...
                "_verify_product_database_state": {
                    "return_value": {"analystResultAuditExact": True}
                },
            ...
            with ExitStack() as stack:
                ...
                receipt = gate.run_analyst_product_qualification_gate(
                    repository_root=REPOSITORY_ROOT,
                    checked_head="3" * 40,
        ...
        self.assertEqual(
            ...
            "private-analyst-product-qualification",
        )
