...
class _Service:
    def __init__(self) -> None:
        self.submitted = []
        self.cancelled = []
        self.view = AnalystAnswerJobView(
            request_id="analyst-answer-" + "1" * 32,
            status="queued",
        )
        self.submit_error: AnalystAnswerServiceError | None = None
...
class AnalystAnswerApiTests(HealthServerTestCase):
    def setUp(self) -> None:
        self.service = _Service()
        self.analyst_answer_service = self.service
        super().setUp()

    def test_submit_status_cancel_and_health_capability(self) -> None:
        status, _, response = self._request("/v1/health")
        self.assertEqual(status, 200)
        self.assertTrue(json.loads(response)["capabilities"]["analystAnswers"])
        ...
        self.assertEqual(request.question, "What was approved?")
        ...
        path = f"/v1/analyst-answers/{self.service.view.request_id}"
        ...
        self.assertEqual(self.service.cancelled, ["analyst-answer-" + "1" * 32])
    ...
    def test_invalid_request_and_unknown_identity_fail_closed(self) -> None:
        ...
        status, headers, body = self._request(
            "/v1/analyst-answers/analyst-answer-" + "9" * 32
        )
        self.assert_error(
            status,
            headers,
    ...
    def test_capacity_error_is_safe_retryable_and_methods_are_exact(self) -> None:
        self.service.submit_error = AnalystAnswerServiceError(
            429,
        ...
        self.assert_error(
            ...
            body,
            expected_status=429,
            code="ANALYST_ANSWER_CAPACITY",
            message="Cited-answer capacity is temporarily unavailable.",
            retryable=True,
        )
