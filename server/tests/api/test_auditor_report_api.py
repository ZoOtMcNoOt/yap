...
class _Service:
    def __init__(self) -> None:
        self.submitted = []
        self.cancelled = []
        self.view = AuditorReportJobView(
            request_id="auditor-report-" + "1" * 32,
            status="queued",
        )
        self.submit_error: AuditorReportServiceError | None = None
...
class AuditorReportApiTests(HealthServerTestCase):
    def setUp(self) -> None:
        self.service = _Service()
        self.auditor_report_service = self.service
        super().setUp()

    def test_submit_status_cancel_and_health_capability(self) -> None:
        status, _, response = self._request("/v1/health")
        self.assertEqual(status, 200)
        self.assertTrue(json.loads(response)["capabilities"]["auditorReports"])
        ...
        self.assertEqual(request.focus, "Helios release limit")
        self.assertEqual(request.maximum_findings, 1)
        ...
        path = f"/v1/auditor-reports/{self.service.view.request_id}"
        ...
        self.assertEqual(self.service.cancelled, ["auditor-report-" + "1" * 32])
    ...
    def test_invalid_request_and_unknown_identity_fail_closed(self) -> None:
        ...
        status, headers, body = self._request(
            "/v1/auditor-reports/auditor-report-" + "9" * 32
        )
        self.assert_error(
            status,
            headers,
    ...
    def test_capacity_error_is_safe_retryable_and_methods_are_exact(self) -> None:
        self.service.submit_error = AuditorReportServiceError(
            429,
        ...
        self.assert_error(
            ...
            body,
            expected_status=429,
            code="AUDITOR_REPORT_CAPACITY",
            message="Audit-report capacity is temporarily unavailable.",
            retryable=True,
        )

        status, headers, body = self._request("/v1/auditor-reports", method="GET")
        self.assert_error(
            status,
            headers,
            body,
