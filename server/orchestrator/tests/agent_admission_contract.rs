...
fn hot_work_yields_after_eight_dispatches_to_ready_background_work() {
    ...
    for tick in 0..9 {
        ...
        order.push(admitted[0].clone());
        let index = admitted[0]
            .strip_prefix("agent-request-")
            .unwrap()
            .parse::<usize>()
            .unwrap();
        scheduler
            .complete(&admitted[0], &cancellation_token(index))
            .unwrap();
    }
...
fn deadline_and_cancellation_are_acknowledged_without_releasing_active_early() {
    ...
    let queued = work(1, "bob", AgentRole::Scribe, Duration::from_secs(5));
    assert_eq!(
