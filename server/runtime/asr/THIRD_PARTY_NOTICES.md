# Yap ASR Runtime Notices

The isolated batch-ASR reference worker uses these pinned third-party artifacts:
...
- `CohereLabs/cohere-transcribe-03-2026`, developed by Cohere and Cohere Labs,
  ...
  `licenses/APACHE-2.0.txt` in the reference image.
- `nvidia/nemotron-3.5-asr-streaming-0.6b`, consumed directly from the
  canonical immutable revision in `server/nemotron-model-pool.lock.json` for
  the Nemotron fixed/dynamic reference worker. The model is governed by
  OpenMDW-1.1. The worker stages no mirror or converted derivative, and the
  complete agreement is shipped at
  `licenses/NEMOTRON_OPENMDW-1.1.txt` in the reference image.
