export async function registerLiveSessionEventListeners(_tauri, options = {}) {
  ...
  globalThis.__yapLiveSessionEventListeners = state;
  state.cleanup = async () => {
...
export async function waitForLiveSessionSavedEvent(_tauri, options = {}) {
  ...
  while (true) {
    ...
    if (!state || !Array.isArray(state.saved)) {
      throw new Error("Live-session event listener state is unavailable while waiting for a saved event.");
    }
