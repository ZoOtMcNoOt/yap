...
fn finalize_owned_work_before_quit(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    let analyst_owner = app.state::<crate::analyst_answer::AnalystAnswerOwner>();
    if let Err(error) = cancel_analyst_answers_before_quit(&analyst_owner) {
        crate::diagnostics::log(&format!(
            "cited-answer shutdown cancellation was incomplete: {error}"
        ));
    }
    let coordinator_owner = app.state::<crate::coordinator_bundle::CoordinatorBundleOwner>();
    if let Err(error) = cancel_coordinator_bundles_before_quit(&coordinator_owner) {
        crate::diagnostics::log(&format!(
    ...
    let curator_owner = app.state::<crate::curator_proposal::CuratorProposalOwner>();
    if let Err(error) = cancel_curator_proposals_before_quit(&curator_owner) {
        crate::diagnostics::log(&format!(
            "knowledge-proposal shutdown cancellation was incomplete: {error}"
        ));
    }
...
pub(super) fn cancel_analyst_answers_before_quit(
    owner: &crate::analyst_answer::AnalystAnswerOwner,
) -> Result<usize, String> {
    tauri::async_runtime::block_on(async {
        tokio::time::timeout(Duration::from_secs(5), owner.cancel_active_requests())
            .await
            .map_err(|_| {
                "cited-answer shutdown cancellation exceeded its bounded wait".to_string()
            })?
    })
}

pub(super) fn cancel_coordinator_bundles_before_quit(
    owner: &crate::coordinator_bundle::CoordinatorBundleOwner,
) -> Result<usize, String> {
    tauri::async_runtime::block_on(async {
        tokio::time::timeout(Duration::from_secs(5), owner.cancel_active_requests())
            .await
...
pub(super) fn cancel_curator_proposals_before_quit(
    owner: &crate::curator_proposal::CuratorProposalOwner,
) -> Result<usize, String> {
    tauri::async_runtime::block_on(async {
        tokio::time::timeout(Duration::from_secs(5), owner.cancel_active_requests())
            .await
            .map_err(|_| {
                "knowledge-proposal shutdown cancellation exceeded its bounded wait".to_string()
            })?
    })
}
