...
#[test]
fn student_shutdown_enters_the_async_runtime_on_its_worker_thread() {
    let owner = crate::student_question::StudentQuestionOwner::new();
    let cancelled = std::thread::spawn(move || cancel_student_questions_before_quit(&owner))
        .join()
...
#[test]
fn archivist_shutdown_enters_the_async_runtime_on_its_worker_thread() {
    let owner = crate::archivist_ingestion::ArchivistIngestionOwner::new();
    let cancelled = std::thread::spawn(move || cancel_archivist_ingestions_before_quit(&owner))
        .join()
        .expect("archivist shutdown worker panicked")
        .expect("empty archivist owner failed shutdown");
