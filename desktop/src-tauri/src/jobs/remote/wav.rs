...
pub(super) const MAX_WAV_CHUNKS: usize = 4_096;
...
pub(super) fn inspect_pcm_wav(
    ...
    if &header[0..4] != b"RIFF" || &header[8..12] != b"WAVE" {
        return Err("Imported recordings must use canonical RIFF/WAVE input".into());
    }
    ...
    let mut container_overhead = 12_u64;
        ...
        .is_some_and(|end| end <= declared_end)
    {
        ...
        chunk_count = chunk_count
            .checked_add(1)
            .ok_or_else(|| "imported WAV RIFF chunk count overflowed".to_string())?;
        if chunk_count > MAX_WAV_CHUNKS {
            return Err("imported WAV contains too many RIFF chunks".into());
        }
        source
        ...
        container_overhead = container_overhead
            .checked_add(8)
            .ok_or_else(|| "imported WAV container overhead overflowed".to_string())?;
        if &chunk_header[0..4] == b"fmt " {
            ...
            if data.is_some() {
                return Err("imported WAV contains multiple data chunks".into());
            }
            container_overhead = container_overhead
    ...
    if !format_valid {
        return Err("Imported recordings require mono signed PCM16 WAV at 16 kHz".into());
    }
    let data = data.ok_or_else(|| "imported WAV has no data chunk".to_string())?;
    validate_pcm_data_bytes(data.data_bytes)?;
    let exact_container_overhead = data
        .source_bytes
        .checked_sub(data.data_bytes)
        .ok_or_else(|| "imported WAV data exceeds its container".to_string())?;
    if exact_container_overhead != container_overhead
        || exact_container_overhead > MAX_WAV_CONTAINER_OVERHEAD_BYTES
    {
        return Err("imported WAV container metadata is too large".into());
    }
    Ok(data)
...
pub(super) fn validate_pcm_data_bytes(data_bytes: u64) -> Result<(), String> {
    ...
    if data_bytes > MAX_JOB_PCM_BYTES {
        return Err("Imported recordings accept at most four hours of PCM audio".into());
    }
    Ok(())
}
